...
public abstract class LPKResource {
	 ...
	 * @return
	 */
	public static BufferedImage openImage(String fileName, String resName) {
		byte[] buffer = null;
		try {
			buffer = LPKResource.openResource(fileName, resName);
			Image img = GraphicsUtils.toolKit.createImage(buffer);
			GraphicsUtils.waitImage(img);
			return GraphicsUtils.getBufferImage(img);
		} catch (Exception e) {
			e.printStackTrace();
