...
public class SHA256 {
	...
	public void _block(LongArray words) {
		...
		for (i = 0; i < 64; i++) {
				...
				tmp = w.items[i & 15] = Convert
						.get(((Convert.MOVE_RightUShift(a, 7)
								...
								+ (Convert.MOVE_RightUShift(b, 17)
										...
										^ Convert.MOVE_RightUShift(b, 10)
										^ b << 15 ^ b << 13) + w.items[i & 15] + w.items[(i + 9) & 15]) | 0);
			}

			tmp = (tmp
					+ h7
					+ (Convert.MOVE_RightUShift(h4, 6)
							^ Convert.MOVE_RightUShift(h4, 11)
							^ Convert.MOVE_RightUShift(h4, 25) ^ h4 << 26
							^ h4 << 21 ^ h4 << 7) + (h6 ^ h4 & (h5 ^ h6)) + k.items[i]);
			h7 = h6;
			h6 = h5;
			h5 = h4;
			h4 = h3 + (int) (tmp | 0);
			...
			h0 = Convert
					.get((tmp + ((h1 & h2) ^ (h3 & (h1 ^ h2))) + (Convert
							.MOVE_RightUShift(h1, 2)
							^ Convert.MOVE_RightUShift(h1, 13)
							^ Convert.MOVE_RightUShift(h1, 22)
							^ h1 << 30
							^ h1 << 19 ^ h1 << 10)) | 0);
		}
