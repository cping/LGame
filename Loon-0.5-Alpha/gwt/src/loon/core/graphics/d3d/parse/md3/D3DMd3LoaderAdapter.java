...
public class D3DMd3LoaderAdapter implements D3DIMdLoaderAdapter {
	...
	public void setVerticeNumber(int surfaceId, int verticeNumber) {
		...
		for (int i = 0; i < frameList.size(); i++) {
			...
			obj.mMesh[0].mVertices = FloatBuffer
					.allocate(D3DMesh.nbFloatPerVertex * verticeNumber);
	...
	@Override
	public void addVertexCoords(int surfaceId, int frameId, int vertexId,
	...
	@Override
	public void addNormalCoords(int surfaceId, int frameId, int vertexId,
	...
	public void addTexCoords(int surfaceId, int vertexId, float u, float v) {
		...
		for (int i = 0; i < frameList.size(); i++) {
			D3DObject obj = frameList.get(i);
			obj.mMesh[0].mVertices.put(vertexId * D3DMesh.nbFloatPerVertex + 6,
					u);
			obj.mMesh[0].mVertices.put(vertexId * D3DMesh.nbFloatPerVertex + 7,
					v);
		}
	}

	@Override
	public void addTriangleIndices(int surfaceId, int triangleId, int a, int b,
			int c) {
		ArrayList<D3DObject> frameList = surfaceList.get(surfaceId);
	...
	@Override
	public void addTag(int frameId, int tagId, String tagName, float x,
			float y, float z, float rotMatrix[]) {
		...
		if (((fileName.toUpperCase().contains("LOWER")) && tagName
				.equals(D3DMdLoader.TAG_TORSO))
				|| ((fileName.toUpperCase().contains("UPPER")) && tagName
						.equals(D3DMdLoader.TAG_HEAD))) {
			float v[][];
