...
public class L2DTargetPoint {
	...
	public void update()
	{
		...
		final float FRAME_TO_MAX_SPEED = TIME_TO_MAX_SPEED * FRAME_RATE  ;
		...
		if( dx == 0 && dy == 0 ) return ;
		float d = (float) MathUtils.sqrt( dx*dx + dy*dy ) ;
		...
		float a = (float) MathUtils.sqrt( ax*ax + ay*ay ) ;
...

		{
			...
			float max_v = 0.5f * (MathUtils.sqrt( MAX_A*MAX_A + 16*MAX_A * d - 8*MAX_A * d ) - MAX_A ) ;
			float cur_v =  MathUtils.sqrt( faceVX*faceVX + faceVY*faceVY ) ;
