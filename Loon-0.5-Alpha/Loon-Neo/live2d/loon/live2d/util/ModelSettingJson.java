...
public class ModelSettingJson implements ModelSetting {
	...
	public ModelSettingJson(String path) {
	...
	public int getMotionFadeIn(String name, int n) {
		return (!existMotionFadeIn(name, n)) ? 1000 : json.get(MOTION_GROUPS)
				.get(name).get(n).get(FADE_IN).toInt();// json.motion_group[name][n].fade_in
	}

	@Override
	public int getMotionFadeOut(String name, int n) {
		return (!existMotionFadeOut(name, n)) ? 1000 : json.get(MOTION_GROUPS)
				.get(name).get(n).get(FADE_OUT).toInt();// json.motion_group[name][n].fade_out
	}
	...
	public String[] getMotionGroupNames() {
		if (json.get(MOTION_GROUPS) == null)
			return null;
		Object[] keys = json.get(MOTION_GROUPS).getMap(null).keys;
	...
	@Override
	public boolean getLayout(ListMap<String, Float> layout) {
		if (json.get(LAYOUT) == null)
			return false;

		ListMap<String, Value> map = json.get(LAYOUT).getMap(null);
	...
	public String[] getSoundPaths() {
		...
		TArray<String> ret = new TArray<String>();
		ListMap<String, Value> map = json.get(MOTION_GROUPS).getMap(null);
		for (int j = 0; j < map.size; j++) {
			TArray<Value> motions = (map.getValueAt(j)).getVector(null);
			if (motions != null) {
