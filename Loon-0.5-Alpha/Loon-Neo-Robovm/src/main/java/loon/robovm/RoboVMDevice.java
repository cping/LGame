...
public enum RoboVMDevice {
	...
	RoboVMDevice(String machineString, int ppi) {
		this.machineString = machineString;
		this.ppi = ppi;
	}
	
	public static RoboVMDevice getDevice (String m) {
		for (RoboVMDevice device : values()) {
			if (device.machineString.equalsIgnoreCase(m)) return device;
		}
		return null;
	}
}
