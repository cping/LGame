...
public class JavaSEGL20 implements loon.core.graphics.opengl.GL20 {
	...
	private void ensureBufferCapacity(int numBytes) {
		if (buffer == null || buffer.capacity() < numBytes) {
			buffer = NativeSupport.newByteBuffer(numBytes);
	...
	private FloatBuffer toFloatBuffer(float v[], int offset, int count) {
		...
		floatBuffer.clear();
		NativeSupport.copy(v, floatBuffer, offset, count);
		return floatBuffer;
	}
	...
	private IntBuffer toIntBuffer(int v[], int offset, int count) {
		ensureBufferCapacity(count << 2);
		floatBuffer.clear();
		NativeSupport.copy(v, offset, intBuffer, count);
		return intBuffer;
	}
	...
	public void glDrawElements(int mode, int count, int type, Buffer indices) {
		if (indices instanceof ShortBuffer
				&& type == loon.core.graphics.opengl.GL20.GL_UNSIGNED_SHORT)
			...
			GL11.glDrawElements(mode, ((ByteBuffer) indices).asShortBuffer()); // FIXME
				...
				&& type == loon.core.graphics.opengl.GL20.GL_UNSIGNED_BYTE)
	...
	public void glTexImage2D(int target, int level, int internalformat,
		...
		if (pixels == null)
			GL11.glTexImage2D(target, level, internalformat, width, height,
					border, format, type, (ByteBuffer) null);
	...
	public void glUniformMatrix2fv(int location, int count, boolean transpose,
		...
		GL20.glUniformMatrix2(location, transpose,
				toFloatBuffer(value, offset, count << 2));
	}
	...
	public void glUniformMatrix3fv(int location, int count, boolean transpose,
			float[] value, int offset) {
		GL20.glUniformMatrix3(location, transpose,
				toFloatBuffer(value, offset, count * 9));
	}
	...
	public void glUniformMatrix4fv(int location, int count, boolean transpose,
			float[] value, int offset) {
		GL20.glUniformMatrix4(location, transpose,
				toFloatBuffer(value, offset, count << 4));
	}
		...
		if (buffer instanceof ByteBuffer) {
			if (type == GL_BYTE)
				GL20.glVertexAttribPointer(indx, size, false, normalized,
						stride, (ByteBuffer) buffer);
			else if (type == GL_UNSIGNED_BYTE)
				GL20.glVertexAttribPointer(indx, size, true, normalized,
						stride, (ByteBuffer) buffer);
			else if (type == GL_SHORT)
				GL20.glVertexAttribPointer(indx, size, false, normalized,
						stride, ((ByteBuffer) buffer).asShortBuffer());
			...
			if (type == GL_FLOAT)
				...
				throw new RuntimeException("Can't use "
						+ buffer.getClass().getName() + " with type " + type
						+ " with this method.");
		} else
			throw new RuntimeException("Can't use "
					+ buffer.getClass().getName()
