...
public class BigNumber {
	...
	public LongArray toBits(long len) {
		this.fullReduce();
		len = Convert.OR(Convert.OR(len, this.exponent).longValue(),
				this.bitLength()).longValue();

		int i = (int) Math.floor((len - 1) / 24);

		long e = (len + 7 & -8)
				% Convert.OR(this.radix, this.radix).longValue();
	...
	public long testBit(int bitIndex) {
		...
		return Convert.MOVE_RightUShift(this.limbs.get(limbIndex),
				bitIndexInLimb) & 1;
	}
	...
	public BigNumber fromBits(LongArray bits) {
		...
		long l = Math.min(Convert.OR(this.bitLength(), 0x100000000l)
				.longValue(), BitArray.bitLength(bits)), e = l
