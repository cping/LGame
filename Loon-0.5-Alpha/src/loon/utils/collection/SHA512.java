	...
	public void _precompute() {
		...
		outer: for (; i < 80; prime++) {
			...
			if (i < 8) {
				...
				this._init.set(
						i * 2 + 1,
						Convert.MOVE_LeftShift(frac2(Math.pow(prime, 1d / 2d)),
								24) | this._initr[i]);
			}
		...
		for (i = 0; i < 80; i++) {
			if (i < 16) {
				...
				long gamma0l = ((Convert.MOVE_LeftShift(gamma0xh, 31) | Convert
						...
						.MOVE_LeftShift(gamma0xh, 25) | Convert
						.MOVE_RightUShift(gamma0xl, 7)));
				...
				long gamma1l = ((Convert.MOVE_LeftShift(gamma1xh, 13) | Convert
						...
						.MOVE_LeftShift(gamma1xh, 26) | Convert
						.MOVE_RightUShift(gamma1xl, 6)));
				...
				wrh = gamma0h
						+ wr7h
						+ (Convert.MOVE_RightUShift(wrl, 0) < Convert
								.MOVE_RightUShift(gamma0l, 0) ? 1 : 0);
				// 2038397261,434469372,16
				wrl += gamma1l;
				wrh += gamma1h
						+ (Convert.MOVE_RightUShift(wrl, 0) < Convert
								.MOVE_RightUShift(gamma1l, 0) ? 1 : 0);
				// 2038397261,434469372,16
				wrl += wr16l;

				wrh += wr16h
						+ (Convert.MOVE_RightUShift(wrl, 0) < Convert
								.MOVE_RightUShift(wr16l, 0) ? 1 : 0);
			...
			long sigma0h = (Convert.MOVE_LeftShift(al, 4) | Convert
					.MOVE_RightUShift(ah, 28))
					^ (Convert.MOVE_LeftShift(ah, 30) | Convert
							.MOVE_RightUShift(al, 2))
					^ (Convert.MOVE_LeftShift(ah, 25) | Convert
							.MOVE_RightUShift(al, 7));
			long sigma0l = (Convert.MOVE_LeftShift(ah, 4) | Convert
					.MOVE_RightUShift(al, 28))
					^ (Convert.MOVE_LeftShift(al, 30) | Convert
							.MOVE_RightUShift(ah, 2))
					^ (Convert.MOVE_LeftShift(al, 25) | Convert
							.MOVE_RightUShift(ah, 7));

			long sigma1h = (Convert.MOVE_LeftShift(el, 18) | Convert
					.MOVE_RightUShift(eh, 14))
					^ (Convert.MOVE_LeftShift(el, 14) | Convert
							.MOVE_RightUShift(eh, 18))
					^ (Convert.MOVE_LeftShift(eh, 23) | Convert
							.MOVE_RightUShift(el, 9));
			long sigma1l = (Convert.MOVE_LeftShift(eh, 18) | Convert
					.MOVE_RightUShift(el, 14))
					^ (Convert.MOVE_LeftShift(eh, 14) | Convert
							.MOVE_RightUShift(el, 18))
					^ (Convert.MOVE_LeftShift(el, 23) | Convert
							.MOVE_RightUShift(eh, 9));
			...
			long t2h = sigma0h
					+ majh
					+ (Convert.MOVE_RightUShift(t2l, 0) < Convert
							.MOVE_RightUShift(sigma0l, 0) ? 1 : 0);
