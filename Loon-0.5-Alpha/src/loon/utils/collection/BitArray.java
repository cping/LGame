...
public class BitArray {
	...
	public static long extract(LongArray a, int bstart, int blength) {
		...
		if (((bstart + blength - 1 ^ bstart) & -32) > 0) {
			x = Convert.MOVE_LeftShift(a.get(bstart / (int) (32 | 0)),
					(32 - sh))
					^ Convert.MOVE_RightUShift(
							a.get(bstart / 32 + (int) (1 | 0)), sh);
		} else {
	...
	public static LongArray _shiftRight(LongArray a, long shift, long carry,
		...
		for (i = 0; i < a.length; i++) {
			out.push((int) (carry | Convert.MOVE_RightUShift(a.get(i),
					(int) shift)));
	...
	public static LongArray byteswapM(LongArray a) {
		...
		for (i = 0; i < a.length; ++i) {
			v = a.get(i);
			a.set(i,
					Convert.MOVE_RightUShift(v, 24)
							| (Convert.MOVE_RightUShift(v, 8) & m)
							| ((v & m) << 8) | (v << 24));
		}
		return a;
	}
