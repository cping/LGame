...
public class MP3Player {
	...
	public MP3Player(InputStream stream, AudioDevice device) throws JavaLayerException {
		if (stream instanceof ByteArrayInputStream) {
			try {
				int nRead;
				final ByteArrayInputStream inputStream = (ByteArrayInputStream) stream;
				...
				buffer.flush();
				_byteArrays = buffer.toByteArray();
			} catch (Exception e) {
				e.printStackTrace();
			}
		} else {
	...
	public void resetStream() throws JavaLayerException {
		if (_byteArrays != null) {
			if (bitstream != null) {
				bitstream.close();
			}
			bitstream = new Bitstream(new ByteArrayInputStream(_byteArrays));
			if (audio == null) {
				audio = FactoryRegistry.systemRegistry().createAudioDevice();
