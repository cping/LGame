...
public class LPad extends LComponent {
	...
	public LPad(int x, int y, float scale) {
		this(x, y, GraphicsUtils.loadBitmap(LSystem.FRAMEWORK_IMG_NAME
				+ "pad_ui_back.png"), GraphicsUtils
				.loadBitmap(LSystem.FRAMEWORK_IMG_NAME + "pad_ui_fore.png"),
				GraphicsUtils.loadBitmap(LSystem.FRAMEWORK_IMG_NAME
						+ "pad_ui_dot.png"), scale);
	}

	public LPad(int x, int y, Bitmap b, Bitmap f, Bitmap d, float scale) {
	...
	public void createUI(LGraphics g, int x, int y, LComponent component,
		...
		g.drawBitmap(back, x, y, backWidth, backHeight);
		if (isClick) {
			if (angle < 360) {
				angle += 1;
			} else {
			...
			g.drawBitmap(dot, x + centerX, y + centerY, dotWidth, dotHeight,
					angle);
		}
		g.drawBitmap(fore, (int) (x + (backWidth - baseWidth) * 0.5f),
