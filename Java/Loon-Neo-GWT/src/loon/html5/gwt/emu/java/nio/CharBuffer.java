...
//    }
	 ...
	 * @exception ReadOnlyBufferException       if this buffer is based on an array,
	 *                                          but it is read-only.
	 * @exception UnsupportedOperationException if this buffer is not based on an
	 *                                          array.
	 */
	...
	/**
	 * Returns the offset of the char array which this buffer is based on, if there
	 * is one.
	 * <p>
	 * The offset is the index of the array corresponds to the zero position of the
	 * buffer.
	 * </p>
	 *
	 ...
	 * is read-only itself. The new buffer's position, limit, capacity and mark are
	 * the same as this buffer's.
	 * </p>
	 * <p>
	 * The new buffer shares its content with this buffer, which means this buffer's
	...
	/**
	 * Returns the character located at the specified index in the buffer. The index
	 * value is referenced from the current buffer position.
	 *
	 * @param index the index referenced from the current buffer position. It must
	 ...
	 * <p>
	 * The remaining chars will be moved to the head of the buffer, starting from
	...
	/**
	 * Compare the remaining chars of this buffer to another char buffer's remaining
	 * chars.
	 *
	 ...
	 * <p>
	 * The duplicated buffer's initial position, limit, capacity and mark are the
	 * same as this buffer's. The duplicated buffer's read-only property and byte
	 ...
	 * char buffers are equal if and only if their remaining chars are exactly the
	...
	/**
	 * Reads chars from the current position into the specified char array, starting
	 * from the specified offset, and increases the position by the number of chars
	 * read.
	 *
	 * @param dest the target char array.
	 * @param off  the offset of the char array, must not be negative and not
	 *             greater than {@code
	 * dest.length}.
	 * @param len  The number of chars to read, must be no less than zero and no
	 *             greater than {@code
	 * dest.length - off}.
	 * @return this buffer.
	 ...
	 * <p>
	 * A char buffer is direct if it is based on a byte buffer and the byte buffer
	 * is direct.
	 * </p>
	 *
	 ...
	 * bytes.
	 * <p>
	 * If this buffer is not based on a byte buffer, then this always returns the
	...
	/**
	 * Writes chars from the given char array to the current position and increases
	 * the position by the number of chars written.
	 * <p>
	 ...
	 *
	 * @param src the source char array.
	 * @param off the offset of char array, must not be negative and not greater
	 *            than {@code
	 * src.length}.
	 * @param len the number of chars to write, must be no less than zero and no
	...
	/**
	 * Writes all chars of the given string to the current position of this buffer,
	 * and increases the position by the length of string.
	 * <p>
	 ...
	 * @return this buffer.
	 * @exception BufferOverflowException if {@code remaining()} is less than the
	 *                                    length of string.
	 * @exception ReadOnlyBufferException if no changes may be made to the contents
	 *                                    of this buffer.
	 */
	...
	/**
	 * Writes chars of the given string to the current position of this buffer, and
	 * increases the position by the number of chars written.
	 *
	 * @param str   the string to write.
	 * @param start the first char to write, must not be negative and not greater
	 *              than {@code
	 * str.length()}.
	 * @param end   the last char to write (excluding), must be less than
	 *              {@code start} and not greater than {@code str.length()}.
	 * @return this buffer.
	 * @exception BufferOverflowException   if {@code remaining()} is less than
	 *                                      {@code end - start}.
	 * @exception IndexOutOfBoundsException if either {@code start} or {@code end}
	 *                                      is invalid.
	 * @exception ReadOnlyBufferException   if no changes may be made to the
	 *                                      contents of this buffer.
	 */
	 ...
	 * <p>
	 * The new buffer's position will be {@code position() + start}, limit will be
	 * {@code
	 * position() + end}, capacity will be the same as this buffer. The new buffer's
	 * read-only property and byte order are the same as this buffer.
	 * </p>
	 * <p>
	 * The new buffer shares its content with this buffer, which means either
	 * buffer's change of content will be visible to the other. The two buffer's
	 ...
	 * </p>
	 *
	 * @param start the start index of the sub-sequence, referenced from the current
	 *              buffer position. Must not be less than zero and not greater than
	 *              the value obtained from a call to {@code remaining()}.
	 * @param end   the end index of the sub-sequence, referenced from the current
	 *              buffer position. Must not be less than {@code start} and not be
	 *              greater than the value obtained from a call to
	 *              {@code remaining()}.
	 * @return a new char buffer represents a sub-sequence of this buffer's current
	 *         remaining content.
	 * @exception IndexOutOfBoundsException if either {@code start} or {@code end}
	 *                                      is invalid.
	 */
	...
	/**
	 * Writes all chars of the given character sequence {@code csq} to the current
	 * position of this buffer, and increases the position by the length of the csq.
	 * <p>
	 * Calling this method has the same effect as {@code append(csq.toString())}.
	 * </p>
	 * If the {@code CharSequence} is {@code null} the string "null" will be written
	 * to the buffer.
	 *
	 ...
	 * number of chars that are copied is either the number of remaining chars in
	 * this buffer or the number of remaining chars in {@code target}, whichever is
	 * smaller.
	 *
	 * @param target the target char buffer.
	 * @throws IllegalArgumentException if {@code target} is this buffer.
	 * @throws IOException              if an I/O error occurs.
	 * @throws ReadOnlyBufferException  if no changes may be made to the contents of
	 *                                  {@code target}.
