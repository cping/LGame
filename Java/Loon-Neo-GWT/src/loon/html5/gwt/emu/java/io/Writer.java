...
public abstract class Writer implements Appendable, Closeable, Flushable {
	...
	/***
	 * Closes this writer. Implementations of this method should free any resources
	...
	/***
	 * Flushes this writer. Implementations of this method should ensure that all
	 * buffered characters are written to the target.
	 * 
	...
	/***
	 * Writes {@code count} characters starting at {@code offset} in {@code buf} to
	 * the target.
	 * 
	 ...
	 * @throws IndexOutOfBoundsException if {@code offset < 0} or {@code count < 0},
	 *                                   or if {@code offset + count} is greater
	 *                                   than the size of {@code buf}.
	 * @throws IOException               if this writer is closed or another I/O
	 *                                   error occurs.
	 */
	public abstract void write(char buf[], int offset, int count) throws IOException;

	/***
	 * Writes one character to the target. Only the two least significant bytes of
	 * the integer {@code oneChar} are written.
	 * 
	...
	/***
	 * Writes {@code count} characters from {@code str} starting at {@code offset}
	 * to the target.
	 * 
	 * @param str    the non-null string containing the characters to write.
	...
	/***
	 * Appends the character {@code c} to the target. This method works the same way
	 * as {@link #write(int)}.
	 * 
	...
	/***
	 * Appends the character sequence {@code csq} to the target. This method works
	 * the same way as {@code Writer.write(csq.toString())}. If {@code csq} is
	 ...
	 * {@code Writer.writer(csq.subsequence(start, end).toString())}. If {@code csq}
	 * is {@code null}, then the specified subsequence of the string "null" will be
	 ...
	 * @throws IOException               if this writer is closed or another I/O
	 *                                   error occurs.
	 * @throws IndexOutOfBoundsException if {@code start > end}, {@code start < 0},
	 *                                   {@code end < 0} or either {@code start} or
	 *                                   {@code end} are greater or equal than the
	 *                                   length of {@code csq}.
	 */
	...
	/***
	 * Returns true if this writer has encountered and suppressed an error. Used by
	 * PrintWriters as an alternative to checked exceptions.
	 */
	boolean checkError() {
		return false;
	}
