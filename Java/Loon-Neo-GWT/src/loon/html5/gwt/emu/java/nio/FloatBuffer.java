 ...
 * </ul>
 */
public final class FloatBuffer extends Buffer implements Comparable<FloatBuffer>, loon.jni.HasArrayBufferView {
	 ...
	 * <p>
	 * The remaining floats will be moved to the head of the buffer, starting from
	 ...
	 * <p>
	 * If {@code other} is not a float buffer then {@code false} is returned. Two
	 * float buffers are equal if and only if their remaining floats are exactly the
	 ...
	 * @param off  the offset of the float array, must not be negative and no
	 *             greater than {@code
	 * dest.length}.
	 * @param len  the number of floats to read, must be no less than zero and no
	 ...
	 *
	 * @return {@code true} if this buffer is based on a float array and provides
...
// }
	 ...
	 * <p>
	 * A float buffer is direct if it is based on a byte buffer and the byte buffer
	 * is direct.
	 * </p>
	 *
	...
	/**
	 * Writes floats from the given float array, starting from the specified offset,
	 * to the current position and increases the position by the number of floats
	 * written.
	 *
	 * @param src the source float array.
	 * @param off the offset of float array, must not be negative and not greater
	 *            than {@code src.length}.
	 * @param len the number of floats to write, must be no less than zero and no
