...
public abstract class Buffer {
	...
	/**
	 * <code>limit - 1</code> is the last element that can be read or written. Limit
	 * must be no less than zero and no greater than <code>capacity</code>.
	 */
	int limit;

	/**
	 * Mark is where position will be set when <code>reset()</code> is called. Mark
	 * is not set by default. Mark is always no less than zero and no greater than
	 * <code>position</code>.
	 */
	int mark = UNSET_MARK;

	/**
	 * The current position of this buffer. Position is always no less than zero and
	 * no greater than <code>limit</code>.
	 */
	int position = 0;
	 ...
	 * changes take place: the current position is reset back to the start of the
	 * buffer, the value of the buffer limit is made equal to the capacity and mark
	 * is cleared.
	 * </p>
	 *
	 * @return this buffer.
	 ...
	 * Flips this buffer.
	 * <p>
	 * The limit is set to the current position, then the position is set to zero,
	 ...
	 * <p>
	 * If the current position in the buffer is in excess of <code>newLimit</code>
	 * then, on returning from this call, it will have been adjusted to be
	 * equivalent to <code>newLimit</code>. If the mark is set and is greater than
	 * the new limit, then it is cleared.
	 * </p>
	 *
