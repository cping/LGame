...
public class JavaANCanvas extends Canvas {
	...
	public Canvas drawOval(float x, float y, float w, float h) {
		context.drawOval(MathUtils.ifloor(x), MathUtils.ifloor(y), MathUtils.iceil(x + w), MathUtils.iceil(y + h),
	...
	public Canvas fillArc(float x, float y, float w, float h, float startAngle, float endAngle) {
		context.drawArc(MathUtils.ifloor(x), MathUtils.ifloor(y), MathUtils.iceil(x + w), MathUtils.iceil(y + h),
				startAngle, endAngle, true, currentState().prepareFill());
		isDirty = true;
		return this;
	}
	...
	private Bitmap createImage() {
		...
		if (img != null) {
			newImage = JavaANImageCachePool.get().find(img.getConfig(), img.getWidth(), img.getHeight());
			android.graphics.Canvas canvas = new android.graphics.Canvas(newImage);
	...
	public Image newSnapshot() {
		if (isMainCanvas()) {
			if (LSystem.base() != null && !((JavaANSetting) LSystem.base().setting).doubleBuffer) {
				return LSystem.base().snapshot();
			}
		}
		return new JavaANImage(gfx, image == null ? Scale.ONE : image.scale(), createImage(),
