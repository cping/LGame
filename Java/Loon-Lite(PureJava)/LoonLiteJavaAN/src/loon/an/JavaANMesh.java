...
public class JavaANMesh implements Mesh {
    ...
    public void renderDrawTriangle(MeshData mesh, int index0, int index1, int index2) {
        ...
        context.save();
        if (transform != null) {
            matrix.setValues(setArrayTransform(transform.m00, transform.m01, transform.m10, transform.m11, transform.tx, transform.ty));
        ...
        matrix.setValues(setArrayTransform(deltaA * dDelta, deltaD * dDelta, deltaB * dDelta, deltaE * dDelta, deltaC * dDelta,
                deltaF * dDelta));
        matrix.setValues(_transform);
