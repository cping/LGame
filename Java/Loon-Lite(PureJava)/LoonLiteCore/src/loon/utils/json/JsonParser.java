...
final class JsonParser {
	...
	<T> T parse(Class<T> clazz) throws JsonParserException {
		...
		if (advanceToken() != Token.EOF) {
			throw createParseException(null, "Expected end of input, got " + _token, true);
		}
	...
	private Object currentValue() throws JsonParserException {
		...
		throw createParseException(null, "Expected JSON value, got " + _token, true);
	}
			...
			if (advanceToken() != Token.ARRAY_END)
				while (true) {
					...
					if (_token != Token.COMMA)
						throw createParseException(null, "Expected a comma or end of the array instead of " + _token,
								true);
					if (advanceToken() == Token.ARRAY_END)
			...
			if (advanceToken() != Token.OBJECT_END)
				while (true) {
					if (_token != Token.STRING) {
						throw createParseException(null, "Expected STRING, got " + _token, true);
					}
					String key = (String) _value;
					if (advanceToken() != Token.COLON) {
						throw createParseException(null, "Expected COLON, got " + _token, true);
					}
					advanceToken();
					...
					if (_token != Token.COMMA) {
						throw createParseException(null, "Expected a comma or end of the object instead of " + _token,
								true);
					}
					if (advanceToken() == Token.OBJECT_END) {
