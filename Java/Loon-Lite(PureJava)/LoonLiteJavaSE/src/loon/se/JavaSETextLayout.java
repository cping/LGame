...
public final class JavaSETextLayout extends loon.font.TextLayout {
	...
	JavaSETextLayout(final String text, final TextFormat format, final TextLayout layout) {
	...
	void paint(Graphics2D gfx, float x, float y, boolean stroke) {
		...
		try {
			...
			final float yoff = y + layout.getAscent();
			if (stroke) {
