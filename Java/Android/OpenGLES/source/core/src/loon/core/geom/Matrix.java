...
public final class Matrix {
	...
	public static class Transform2i {

		public int[][] matrixs = empty();
		...
		public void set(int[][] matrixs) {
			this.matrixs = matrixs;
		}

		public final void mul(final int[][] matrixs) {
			this.matrixs = mul(matrixs, matrixs);
		}

		public final int[] mul(final int[] fpV) {
		...
		public final void zoom(final float scale, final float x, final float y) {
			if (scale != 1f) {
		...
		public final static int[][] empty() {
			int[][] id = { { MathUtils.ONE_FIXED, 0, 0 },
					{ 0, MathUtils.ONE_FIXED, 0 },
					{ 0, 0, MathUtils.ONE_FIXED } };
			return id;
		}

		public final static int[][] def() {
			int[][] id = { { 0, 0, 0 }, { 0, 0, 0 }, { 0, 0, 0 } };
			return id;
		}

		public final static int[][] mul(final int[][] a, int[][] b) {
			int[][] matrixs = def();
			for (int i = 0; i < 3; ++i) {
				for (int j = 0; j < 3; ++j) {
					for (int n = 0; n < 3; ++n) {
						matrixs[i][j] += MathUtils.mul(a[i][n], b[n][j]);
					}
				}
			}
			return matrixs;
		...
		public final static int[] mul(final int[][] a, int[] b) {
			...
			for (int i = 0; i < 3; ++i) {
				for (int j = 0; j < 3; ++j) {
					matrixs[i] += MathUtils.mul(a[i][j], b[j]);
				}
			}
			return matrixs;
		}

		public final static int[][] zoomMatrix(final float scale,
				final float x, final float y) {
			final int mu = (0 == scale) ? Integer.MAX_VALUE : MathUtils
					.fromFloat(1 / scale);
			if (MathUtils.ONE_FIXED == mu) {
				return def();
			}
			final int x_c = MathUtils.fromFloat(x);
			final int y_c = MathUtils.fromFloat(y);
			final int transX = x_c - MathUtils.mul(x_c, mu);
			final int transY = y_c - MathUtils.mul(y_c, mu);
			int[][] zoom = { { mu, 0, transX }, { 0, mu, transY },
					{ 0, 0, MathUtils.ONE_FIXED } };
			return zoom;
		}

		public final static int[][] rotationMatrix(final float alpha,
				final float x, final float y) {
			if (0 == alpha % (2 * MathUtils.PI)) {
				return empty();
			}
			final int cosAlpha = MathUtils.fromDouble(MathUtils.cos(alpha));
			final int sinAlpha = MathUtils.fromDouble(MathUtils.sin(alpha));
			final int x_c = MathUtils.fromFloat(x);
			final int y_c = MathUtils.fromFloat(y);
			final int transX = MathUtils.mul(x_c, MathUtils.ONE_FIXED
					- cosAlpha)
					+ MathUtils.mul(y_c, sinAlpha);
			final int transY = MathUtils.mul(y_c, MathUtils.ONE_FIXED
					- cosAlpha)
					- MathUtils.mul(x_c, sinAlpha);
			int[][] angle = { { cosAlpha, -sinAlpha, transX },
