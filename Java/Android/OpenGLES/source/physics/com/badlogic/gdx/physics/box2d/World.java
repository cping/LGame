...
public final class World implements LRelease {
	...
	public Body createBody(BodyDef def) {
		long bodyAddr = jniCreateBody(addr, def.type.getValue(),
				def.position.x, def.position.y, def.angle,
	...
	private native long jniCreateBody(long addr, int type, float positionX,
			...
			float inertiaScale); /*
								 * b2BodyDef bodyDef; bodyDef.type =
								 * getBodyType(type); bodyDef.position.Set(
								 * positionX, positionY ); bodyDef.angle =
								 * angle; bodyDef.linearVelocity.Set(
								 * linearVelocityX, linearVelocityY );
								 * bodyDef.angularVelocity = angularVelocity;
								 * bodyDef.linearDamping = linearDamping;
								 * bodyDef.angularDamping = angularDamping;
								 * bodyDef.allowSleep = allowSleep;
								 * bodyDef.awake = awake; bodyDef.fixedRotation
								 * = fixedRotation; bodyDef.bullet = bullet;
								 * bodyDef.active = active; bodyDef.gravityScale
								 * = inertiaScale;
								 * 
								 * b2World* world = (b2World*)addr; b2Body* body
								 * = world->CreateBody( &bodyDef ); return
								 * (jlong)body;
								 */
	...
	private long createProperJoint(JointDef def) {
		...
		if (def.type == JointType.WheelJoint) {
			...
			return jniCreateWheelJoint(addr, d.bodyA.addr, d.bodyB.addr,
					d.collideConnected, d.localAnchorA.x, d.localAnchorA.y,
					d.localAnchorB.x, d.localAnchorB.y, d.localAxisA.x,
					d.localAxisA.y, d.enableMotor, d.maxMotorTorque,
					d.motorSpeed, d.frequencyHz, d.dampingRatio);
		}
	...
	private native long jniCreateWheelJoint(long addr, long bodyA, long bodyB,
			...
			float localAxisAY, boolean enableMotor, float maxMotorTorque,
			float motorSpeed, float frequencyHz, float dampingRatio); /*
																	 * b2World*
																	 * world =
																	 * (b2World
																	 * *)addr;
	...
	private native long jniCreateFrictionJoint(long addr, long bodyA,
			...
			float maxForce, float maxTorque); /*
											 ...
											 * b2FrictionJointDef def; def.bodyA
											 ...
											 * localAnchorBY); def.maxForce =
											 * maxForce; def.maxTorque =
											 * maxTorque; return
	...
	private native long jniCreatePrismaticJoint(long addr, long bodyA,
			...
			float motorSpeed); /*
								 * b2World* world = (b2World*)addr;
								 * b2PrismaticJointDef def; def.bodyA =
								 * (b2Body*)bodyA; def.bodyB = (b2Body*)bodyB;
								 * def.collideConnected = collideConnected;
								 * def.localAnchorA = b2Vec2(localAnchorAX,
								 * localAnchorAY); def.localAnchorB =
								 * b2Vec2(localAnchorBX, localAnchorBY);
								 * def.localAxisA = b2Vec2( localAxisAX,
								 * localAxisAY ); def.referenceAngle =
								 * referenceAngle; def.enableLimit =
								 * enableLimit; def.lowerTranslation =
								 * lowerTranslation; def.upperTranslation =
								 * upperTranslation; def.enableMotor =
								 * enableMotor; def.maxMotorForce =
								 * maxMotorForce; def.motorSpeed = motorSpeed;
	...
	private native long jniCreatePulleyJoint(long addr, long bodyA, long bodyB,
			...
			float localAnchorBY, float lengthA, float lengthB, float ratio); /*
																			 * b2World
																			 * *
																			 * world
																			 * =
	...
	private native long jniCreateRevoluteJoint(long addr, long bodyA,
			...
			float maxMotorTorque); /*
									 * b2World* world = (b2World*)addr;
									 * b2RevoluteJointDef def; def.bodyA =
									 * (b2Body*)bodyA; def.bodyB =
									 * (b2Body*)bodyB; def.collideConnected =
									 * collideConnected; def.localAnchorA =
									 * b2Vec2(localAnchorAX, localAnchorAY);
									 * def.localAnchorB = b2Vec2(localAnchorBX,
									 * localAnchorBY); def.referenceAngle =
									 * referenceAngle; def.enableLimit =
									 * enableLimit; def.lowerAngle = lowerAngle;
	...
	private native void jniQueryAABB(long addr, float lowX, float lowY,
			float upX, float upY); /*
									 * b2World* world = (b2World*)addr; b2AABB
									 * aabb; aabb.lowerBound = b2Vec2( lowX,
									 * lowY ); aabb.upperBound = b2Vec2( upX,
									 * upY );
									 * 
	...
	private boolean contactFilter(long fixtureA, long fixtureB) {
		if (contactFilter != null)
			...
			if (fixtures == null || fixtures.get(fixtureA) == null
					|| fixtures.get(fixtureB) == null) {
				return false;
			}
			try {
	 ...
	 */
	public void rayCast(RayCastCallback callback, Vector2f point1,
			Vector2f point2) {
	...
	private native void jniRayCast(long addr, float aX, float aY, float bX,
			float bY); /*
						 * b2World *world = (b2World*)addr;
						 * CustomRayCastCallback callback( env, object );
						 * world->RayCast( &callback, b2Vec2(aX,aY),
						 * b2Vec2(bX,bY) );
						 */

	private Vector2f rayPoint = new Vector2f();
	private Vector2f rayNormal = new Vector2f();
