...
public class MainGame extends DrawableScreen {
	...
	public int add_button(int x, int y, int type, int layers, int tile_base) {
		...
		for (int i = 0; i < this.buttons; i++) {
			if (this.button[i].active != 1) {
				...
				this.button[i].loc.color = LColor.white;
				...
				this.button[i].dest = this.button[i].loc.clone();
	...
	public int add_object(int x, int y, int t) {
		...
		for (int i = 0; i < this.objects; i++) {
			if (this.obj[i].active != 1) {
				...
				this.obj[i].dest = this.obj[i].loc.clone();
				return num2;
			}
		}
		return num2;
	...
	public int add_shot(int x, int y, float angle, float scale, int type,
		...
		for (int i = 0; i < this.shots; i++) {
			if (this.shot[i].active != 1) {
				...
				this.shot[i] = this.ammo[type].shot[type_num].clone();
				...
				this.shot[i].obj.dest = this.shot[i].obj.loc.clone();
	...
	public void get_input() {
		...
		if (Touch.isDown()) {
			this.click_move.set(Touch.getLocation());
		}
		if (this.click != -1) {
			this.click = 0;
		}
	...
	public void load_ammo() {
		...
		index = 1;
		this.ammo[a].shot[index] = this.ammo[a].shot[0].clone();
		this.ammo[a].shot[index].obj.loc.fx = -25f;
		...
		index = 1;
		this.ammo[a].shot[index] = this.ammo[a].shot[0].clone().clone();
	...
	public void set_npc(int n_id, int npc_type, float mod, int t1, int t2) {
		...
		this.npc[index].obj.dest = this.npc[index].obj.loc.clone();
		if (npc_type == 5) {
			if (t1 < 1) {
	...
	private loc_struct update_loc(loc_struct src, loc_struct dst) {
		loc_struct source = src.clone();
		loc_struct dest = dst.clone();
		float num2;
		float num3 = 0f;
		float num16 = 0f;
		...
		if (this.mode == 5) {
			...
			if (this.mode == 6) {
				...
				if (this.mode == 8) {
					...
					// 原版地图渲染方式有性能问题(原本是C++的...),不适合Java下采用,建议改为单独的纹理图片作为背景
	...
	public void loadContent() {
		if (LSystem.type != LSystem.ApplicationType.Android) {
			...
			LImage image = new LImage("assets/tiles.png");
			LImage newImage = image.scaledInstance(576, 960);
			image.dispose();
			// 处理纹理数据
			LTextureData data = GLLoader.getTextureData(newImage);
			// 预处理Alpha通道(默认此项不开启,不开启时若图片中含有Alpha通道,会显示为黑色.开启则处理图像较耗时间.)
			data.setMultipyAlpha(true);
			LTexture texture = new LTexture(data).scale(960, 1600);
