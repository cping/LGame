...
public class SLGTest extends Stage {
	...
	public class Move extends Draw {
		...
		public void setMoveCourse(int moveX, int moveY) {
			...
			if ((movingList[redressX(moveX - 1)][moveY] != moveCount)
					...
					&& (movingList[moveX][redressY(moveY + 1)] != moveCount)
					|| (moveCount + getMapCost(moveX, moveY) > getRoleIdxObject(actionIdx).getMove())) {
	...
	public void removeRole(final BooleanValue gameRunning, final int team, final GameRole e) {
		// 从单元集合中删除角色
	...
	public void attackEnemy(final BooleanValue gameRunning, int team, final Move moveState, final GameRole attacker,
			final GameRole enemy) {
		// 如果目标是敌人
	...
	private LTexture[] unitImages = TextureUtils
			.getSplitTextures(TextureUtils.filterColor("assets/unit.png", new LColor(255, 0, 255)), gameTile, gameTile);

	private LTexture[] iconImages = TextureUtils
			.getSplitTextures(TextureUtils.filterColor("assets/icon.png", new LColor(255, 0, 255)), gameTile, gameTile);

	private LTexture[] listImages = TextureUtils
			.getSplitTextures(TextureUtils.filterColor("assets/list.png", new LColor(255, 0, 255)), gameTile, gameTile);
	...
	public void create() {
		...
		createTeamRole("空罐少女", idCounter, Team.Player, "assets/player.png", 1, 5, 12, 20);
		createTeamRole("猫猫1", idCounter, Team.Player, "assets/player2.png", 0, 4, 11, 20);
		createTeamRole("猫猫2", idCounter, Team.Player, "assets/player2.png", 0, 4, 13, 20);
		// 创建角色:name=躲猫兵团1,id(累加),team=1(敌军),imageindex=2,移动能力3格,x=11,y=4,以下雷同
		createTeamRole("躲猫兵团1", idCounter, Team.Enemy, "assets/enemy1.png", 2, 3, 11, 4);
		createTeamRole("躲猫兵团2", idCounter, Team.Enemy, "assets/enemy1.png", 2, 2, 11, 5);
		createTeamRole("躲猫兵团3", idCounter, Team.Enemy, "assets/enemy1.png", 2, 3, 11, 6);
		createTeamRole("躲猫兵团5", idCounter, Team.Enemy, "assets/enemy1.png", 2, 2, 13, 4);
		createTeamRole("躲猫兵团5", idCounter, Team.Enemy, "assets/enemy1.png", 2, 2, 13, 5);
		createTeamRole("躲猫兵团6", idCounter, Team.Enemy, "assets/enemy1.png", 2, 3, 13, 6);
		createTeamRole("躲猫兵团8", idCounter, Team.Enemy, "assets/enemy1.png", 2, 2, 11, 18);
		createTeamRole("躲猫兵团9", idCounter, Team.Enemy, "assets/enemy1.png", 2, 3, 12, 18);
		createTeamRole("躲猫兵团7", idCounter, Team.Enemy, "assets/enemy1.png", 2, 2, 13, 18);
			...
			case Team.Enemy:
				if (moveState.isAttacking()) {
					...
					if (checkAttackEnemy(attacker, curTileX, curTileY)) {
						// 以索引ID获得敌人并执行攻击
						attackEnemy(gameRunning, Team.Enemy, moveState, attacker, role);
						menuState.hide();
						menu.hide();
						moveState.clear();
						roleIndex.set(-1);
	...
	public void updateRound(final IntValue roleIndex, BooleanValue playerRound, BooleanValue enemyRound,
			final Array<GameRole> playerStack, final Array<GameRole> enemyStack, final Move moveState,
			final State menuState, final Menu menu, final Counter roundCount) {
