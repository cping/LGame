...
public class WebGL20 extends GL20 implements GLExt {
	...
	@JSClass
	static class CacheMap<T extends JSObject> implements JSObject {
		@JSBody(script = "return [undefined];")
		public static native <T extends JSObject> CacheMap<T> create();
		...
		@JSBody(params = {
				"value" }, script = "for (var i = 0; i < this.length; i++) { if (value === this[i]) { return i; } }")
		public native int getKey(T value);
	}

	static final int VERTEX_ATTRIB_ARRAY_COUNT = 5;
	final CacheMap<WebGLProgram> programs = CacheMap.create();
	final CacheMap<WebGLShader> shaders = CacheMap.create();
	final CacheMap<WebGLBuffer> buffers = CacheMap.create();
	final CacheMap<WebGLFramebuffer> frameBuffers = CacheMap.create();
	final CacheMap<WebGLRenderbuffer> renderBuffers = CacheMap.create();
	final CacheMap<WebGLTexture> textures = CacheMap.create();
	final CacheMap<CacheMap<WebGLUniformLocation>> uniforms = CacheMap.create();
	...
	public WebGLContext glc;

	public WebGL20() {
		this(false);
	}

	public WebGL20(boolean checkErrors) {
		super(new Buffers() {
			@Override
	...
	private ArrayBufferView getTypedArray(Buffer buffer, int type, int byteSize) {
		final int bufferElementSize = ConvertUtils.getElementSize(buffer);
		final ArrayBufferView webGLArray = ConvertUtils.getTypedArray(buffer);
		if (byteSize == -1) {
			byteSize = buffer.remaining() * bufferElementSize;
		}
		final int byteOffset = webGLArray.getByteOffset() + buffer.position() * bufferElementSize;
		final ArrayBuffer arrayBuffer = webGLArray.getBuffer();
		switch (type) {
		...
		case WebGLContext.INT:
			return new Int32Array(arrayBuffer, byteOffset, byteSize / 4);
		case WebGLContext.SHORT:
			return new Int16Array(arrayBuffer, byteOffset, byteSize / 2);
		case WebGLContext.BYTE:
			return new Int8Array(arrayBuffer, byteOffset, byteSize);
		default:
	...
	public void init(WebGLContext glc) {
		glc.pixelStorei(WebGLContext.UNPACK_PREMULTIPLY_ALPHA_WEBGL, WebGLContext.ONE);
		this.glc = glc;
			...
			if (enabled != 0 && (useNioBuffer & mask) != 0) {
				...
				if (previousNio != null && previousNio.nioBuffer == data.nioBuffer
					...
					if (boundArrayBuffer != previousNio.webGlBuffer) {
						glc.bindBuffer(WebGLContext.ARRAY_BUFFER, previousNio.webGlBuffer);
					...
					if (boundArrayBuffer != data.webGlBuffer) {
						glc.bindBuffer(WebGLContext.ARRAY_BUFFER, data.webGlBuffer);
						boundArrayBuffer = data.webGlBuffer;
					}
					int elementSize = ConvertUtils.getElementSize(data.nioBuffer);
					int savePosition = data.nioBuffer.position();
					if (data.nioBufferPosition * elementSize < data.stride) {
						data.nioBuffer.position(0);
						glc.bufferData(WebGLContext.ARRAY_BUFFER,
						...
						glc.bufferData(WebGLContext.ARRAY_BUFFER,
								getTypedArray(data.nioBuffer, data.type,
										(data.nioBufferLimit - data.nioBufferPosition) * elementSize),
								WebGLContext.STREAM_DRAW);
	...
	public void glDrawElements(int mode, int count, int type, Buffer indices) {
		...
		if (boundElementArrayBuffer != elementBuffer) {
			glc.bindBuffer(WebGLContext.ELEMENT_ARRAY_BUFFER, elementBuffer);
			boundElementArrayBuffer = elementBuffer;
		}
		glc.bufferData(WebGLContext.ELEMENT_ARRAY_BUFFER, getTypedArray(indices, type, count * getTypeSize(type)),
				WebGLContext.STREAM_DRAW);
	...
	public void glGetBufferParameteriv(int target, int pname, IntBuffer params) {
		params.put(params.position(), ConvertUtils.toNumber(glc.getBufferParameter(target, pname)));
	}
	...
	public void glGetIntegerv(int pname, IntBuffer params) {
		if (pname == GL20.GL_ACTIVE_TEXTURE || pname == GL20.GL_ALPHA_BITS || pname == GL20.GL_BLEND_DST_ALPHA
			...
			Int32Array array = (Int32Array) glc.getParameterv(pname);
			...
			WebGLFramebuffer fbo = (WebGLFramebuffer) glc.getParametero(pname);
			if (fbo == null) {
				params.put(0);
			} else {
	...
	public void glGetTexParameterfv(int target, int pname, FloatBuffer params) {
		params.put(params.position(), ConvertUtils.toNumber(glc.getTexParameter(target, pname)));
	}
	...
	public void glGetUniformfv(int program, int location, FloatBuffer params) {
		Float32Array v = ConvertUtils
				.toFloat32Array(glc.getUniform(programs.get(program), uniforms.get(program).get(location)));
	...
	public void glGetUniformiv(int program, int location, IntBuffer params) {
		Int32Array v = ConvertUtils
				.toInt32Array(glc.getUniform(programs.get(program), uniforms.get(program).get(location)));
		for (int i = 0; i < v.getLength(); i++) {
			params.put(params.position() + i, v.get(i));
		}
	}
	...
	public void glGetVertexAttribfv(int index, int pname, FloatBuffer params) {
		Float32Array v = ConvertUtils.toFloat32Array(glc.getVertexAttrib(index, pname));
		for (int i = 0; i < v.getLength(); i++) {
			params.put(params.position() + i, v.get(i));
		}
	}
	...
	public void glTexImage2D(int target, int level, int internalformat, int format, int type, HTMLImageElement image) {
		glc.texImage2D(target, level, internalformat, format, type, image);
		checkError("texImage2D");
	}

	public void glTexImage2D(int target, int level, int internalformat, int format, int type, HTMLCanvasElement image) {
		glc.texImage2D(target, level, internalformat, format, type, image);
		checkError("texImage2D");
	}
	...
	public void glUniform1fv(int location, int count, FloatBuffer v) {
		...
		glc.uniform1fv(loc, ConvertUtils.copy(v));
	}
	...
	public void glUniform2iv(int location, int count, IntBuffer v) {
		WebGLUniformLocation loc = getUniformLocation(location);
		glc.uniform2iv(loc, ConvertUtils.copy(v));
	}
	...
	public void glUniform3fv(int location, int count, FloatBuffer v) {
		WebGLUniformLocation loc = getUniformLocation(location);
		glc.uniform3fv(loc, ConvertUtils.copy(v));
	}
	...
	public void glUniform3iv(int location, int count, IntBuffer v) {
		WebGLUniformLocation loc = getUniformLocation(location);
		glc.uniform3iv(loc, ConvertUtils.copy(v));
	}
	...
	public void glUniform4iv(int location, int count, IntBuffer v) {
		WebGLUniformLocation loc = getUniformLocation(location);
		glc.uniform4iv(loc, ConvertUtils.copy(v));
	}
	...
	public void glUniformMatrix2fv(int location, int count, boolean transpose, FloatBuffer value) {
		WebGLUniformLocation loc = getUniformLocation(location);
		glc.uniformMatrix2fv(loc, transpose, ConvertUtils.copy(value));
	}
	...
	public void glUniformMatrix3fv(int location, int count, boolean transpose, FloatBuffer value) {
		WebGLUniformLocation loc = getUniformLocation(location);
		glc.uniformMatrix3fv(loc, transpose, ConvertUtils.copy(value));
	}
	...
	public void glVertexAttrib1fv(int indx, FloatBuffer values) {
		glc.vertexAttrib1fv(indx, ConvertUtils.copy(values));
	}
	...
	public void glVertexAttrib2fv(int indx, FloatBuffer values) {
		glc.vertexAttrib2fv(indx, ConvertUtils.copy(values));
	}
	...
	public void glVertexAttrib3fv(int indx, FloatBuffer values) {
		glc.vertexAttrib3fv(indx, ConvertUtils.copy(values));
	}
	...
	public void glVertexAttrib4fv(int indx, FloatBuffer values) {
		glc.vertexAttrib4fv(indx, ConvertUtils.copy(values));
	}
	...
	public void glGetShaderiv(int shader, int pname, int[] params, int offset) {
		if (pname == GL_COMPILE_STATUS)
			params[offset] = glc.getShaderParameterb(shaders.get(shader), WebGLContext.COMPILE_STATUS) ? GL_TRUE
					: GL_FALSE;
		else
