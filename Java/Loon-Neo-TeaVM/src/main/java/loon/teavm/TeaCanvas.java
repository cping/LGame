...
public class TeaCanvas extends Canvas {
	...
	public TeaCanvas(Graphics gfx, TeaImage image) {
		super(gfx, image);
		this.ctx = image.getContext2d();
	...
	public Canvas clip(Path path) {
		assert path instanceof TeaPath;
		((TeaPath) path).replay(ctx);
		ctx.clip();
		return this;
	}
	...
	public Gradient createGradient(Gradient.Config config) {
		return new TeaGradient(ctx, config);
	}
	...
	public Canvas fillText(TextLayout layout, float x, float y) {
		((TeaTextLayout) layout).fill(ctx, x, y);
		isDirty = true;
		return this;
	}
	...
	public Canvas setFillGradient(Gradient gradient) {
		assert gradient instanceof TeaGradient;
		ctx.setFillStyle(((TeaGradient) gradient).gradient);
		return this;
	}

	@Override
	public Canvas setFillPattern(Pattern pattern) {
		assert pattern instanceof TeaPattern;
		ctx.setFillStyle(((TeaPattern) pattern).pattern(ctx));
		return this;
	}
	...
	public Canvas strokeText(TextLayout layout, float x, float y) {
		((TeaTextLayout) layout).stroke(ctx, x, y);
		isDirty = true;
		return this;
	}
	...
	@Override
	protected CanvasRenderingContext2D gc() {
		return ctx;
	}
	...
	private String convertLineCap(LineCap cap) {
		switch (cap) {
		case BUTT:
			return "butt";
		case ROUND:
	...
	private String convertLineJoin(LineJoin join) {
		switch (join) {
		case BEVEL:
			return "bevel";
		case MITER:
