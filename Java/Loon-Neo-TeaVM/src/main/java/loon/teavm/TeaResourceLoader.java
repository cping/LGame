...
public class TeaResourceLoader {
	public final AssetPreloader preloader;
	private final String file;
	private final FileType type;

	public TeaResourceLoader(AssetPreloader preloader, String fileName, FileType type) {
		if (type != FileType.Internal && type != FileType.Classpath) {
			throw new RuntimeException("FileType '" + type + "' Not supported in TeaVM backend");
		}
	...
	public TeaResourceLoader(String path) {
	...
	public OutputStream write(boolean append, int bufferSize) {
		return preloader.write(this, append, bufferSize);
	}
	...
	public TeaResourceLoader[] list() {
		return preloader.listToRes(file);
	}

	public TeaResourceLoader[] list(String suffix) {
	...
	public TeaResourceLoader child(String name) {
		return new TeaResourceLoader(preloader, (file.isEmpty() ? "" : (file + (file.endsWith("/") ? "" : "/"))) + name,
				FileType.Internal);
	}

	public TeaResourceLoader parent() {
		int index = file.lastIndexOf("/");
		String dir = "";
		if (index > 0)
			dir = file.substring(0, index);
		return new TeaResourceLoader(preloader, dir, type);
	}

	public TeaResourceLoader sibling(String name) {
