...
public abstract class SRPGScreen extends Screen implements SRPGType, Runnable {
	...
	protected SRPGDrawView makeDamageExpectView(final SRPGAbilityFactory ab,
		...
		SRPGDamageExpectView view = new SRPGDamageExpectView(ab, srpgField,
				srpgActors, atk, def);
		view.setLocation((getWidth() - view.getWidth()) / 2,
				getHeight() - view.getHeight() - 10);
		return view;
	}
	...
	private LTexture createTempImage() {
		if (messageImage == null) {
			Image tmp = Image
					.createImage(getWidth() - 40, getHeight() / 2 - 20);
	...
	public void centerCamera(int index) {
		...
		centerCamera((actor.drawX() + halfTileWidth) - halfWidth,
				(actor.drawY() + halfTileHeight) - halfHeight);
	}
	...
	private void returningDamageValue(int[] ability, int atk, int def) {
		...
		int[] res = SRPGAbilityFactory.filtedRange(
				SRPGAbilityFactory.filtedAbility(ability,
						attacker.getActorStatus(), false), srpgField,
				attacker.getPosX(), attacker.getPosY(), defender.getPosX(),
				defender.getPosY());
		if (res == null) {
			return;
		}
		...
		if (attacker.getActorStatus().team == 0) {
			...
			srpgDrawView = makeDamageExpectView(
					SRPGAbilityFactory.getInstance(srpgChoiceView
							.getJointContent()), srpgField,
			...
			if (isEnemyView()) {
				...
				srpgDrawView = makeDamageExpectView(
						SRPGAbilityFactory.getInstance(optimizeAbility),
	...
	protected synchronized boolean setDamageValue(int number, int index,
			boolean flag) {
		return setDamageValue(number, index,
				SRPGAbilityOption.getInstance(flag), true);
	}

	protected synchronized boolean setDamageValue(int number, int index,
			boolean flag, boolean flag1) {
		return beforeDamageValue(number, index,
				SRPGAbilityOption.getInstance(flag), flag1);
	}
	...
	protected synchronized void setDamageValueImplement(int abilityIndex,
		...
		srpgDrawView = makeAbilityNameView(ability,
			...
			for (int x = 0; x < area[y].length; x++) {
				...
				if (isBattle) {
					...
					if (damageData.isHit()) {
						if (abilityoption.warp) {
							...
							setEffect(SRPGEffectFactory.getAbilityEffect(
									SRPGEffectFactory.EFFECT_OUT, actor,
									actor.getPosX(), actor.getPosY()));
					...
					if (damageData.isHit() && damageData.getPosX() != -1
						...
						actor.setPos(damageData.getPosX(), damageData.getPosY());
					}
					if (actor.getActorStatus().moveCheckStatus()
							&& damageData.getActorStatus().moveCheckStatus()) {
						actor.setDirection(actor.findDirection(
		...
		if (damagedata != null) {
			...
			this.setLock(true);
			this.srpgDrawView = makeActorMiniStatusView(
					actorObject.getActorStatus(), actorObject);
	...
	protected void setDamage(SRPGDamageData damagedata, SRPGActor actor) {
		...
		if (actor.drawX() - cam_x < tileWidth
				...
				|| (actor.drawY() - cam_y) + tileHeight > getHeight()
						- tileHeight) {
			moveCamera((actor.drawX() + halfTileWidth) - halfWidth,
					(actor.drawY() + halfTileHeight) - halfHeight, 10);
		}
		...
		setEffect(damagedata.getNumberEffect(actor.getPosX() * tileWidth,
		...
		if ((damagedata.getGenre() == GENRE_ALLDAMAGE || damagedata.getGenre() == GENRE_ALLRECOVERY)
			...
			status.mp = damagedata.getActorStatus().mp;
			setEffect(damagedata.getNumberEffect(genre,
					String.valueOf(damagedata.getMP()), actor.getPosX()
	...
	private synchronized void setMove(int i, SRPGActor actor) {
		...
		if (pos == null) {
			...
			srpgPosition.route = srpgMove.moveRoute(actor.getPosX(),
					actor.getPosY(), pos[0], pos[1], move);
	...
	private void moveCPU(int index, boolean flag) {
		...
		SRPGActor actor = srpgActors.find(index);
		if (srpgAI == null) {
			srpgAI = new SRPGAI(srpgField, srpgActors, index,
		...
		if (srpgAI.getAbility() != -1) {
			...
			if (isEnemyView()) {
				setAttackRange(srpgAI.getAbility(), actor.getPosX(),
						actor.getPosY());
				...
				moveCameraCenter(srpgAI.getTargetX() * tileWidth
						...
						+ halfTileHeight, 8);
				waitTime(sleepTime / 2);
				int role = srpgActors.checkActor(srpgAI.getTargetX(),
						srpgAI.getTargetY());
				if (!getTargetTrue(
						SRPGAbilityFactory.getInstance(srpgAI.getAbility()),
						srpgPosition.number, role)) {
					role = -1;
				}
	...
	private void afterCheck() {
		...
		for (int j = 0; j < srpgActors.size(); j++) {
			...
			boolean flag = false;
			SRPGFieldElement element = srpgField.getPosMapElement(
					actor.getPosX(), actor.getPosY());
			// 特殊地形判定
	...
	protected synchronized void defaultCommand() {
		...
		setAttackRange(actor.getActorStatus().ability, actor.getPosX(),
				actor.getPosY());
		if (srpgChoiceView == null) {
	...
	private synchronized void abilityTarget() {
		...
		if (pos != null) {
			...
			case -1:
			case 1:
				setAttackRange(srpgPosition.ability,
						srpgActors.find(srpgPosition.number).getPosX(),
						srpgActors.find(srpgPosition.number).getPosY());
				procFlag = PROC_ABILITYTARGET;
	...
	public synchronized void draw(GLEx g) {
		...
		if (procFlag == PROC_ABILITYSELECT
				&& srpgPosition.ability != srpgChoiceView.getJointContent()) {
			setAttackRange(srpgChoiceView.getJointContent(),
					srpgActors.find(srpgPosition.number).getPosX(), srpgActors
							.find(srpgPosition.number).getPosY());
			srpgPosition.ability = srpgChoiceView.getJointContent();
		}
		...
		if (procFlag == PROC_COUNTER && srpgPosition.counter
				...
				&& srpgPosition.ability != srpgChoiceView.getJointContent()) {
			setTargetRange(srpgChoiceView.getJointContent(),
					srpgActors.find(srpgPosition.enemy).getPosX(), srpgActors
							.find(srpgPosition.enemy).getPosY());
		}
