...
public class QRUtil {
	...
	private static final int[][] PATTERN_POSITION_TABLE = { {}, { 6, 18 }, { 6, 22 }, { 6, 26 }, { 6, 30 }, { 6, 34 },
	...
	private static final int G18 = (1 << 12) | (1 << 11) | (1 << 10) | (1 << 9) | (1 << 8) | (1 << 5) | (1 << 2)
			| (1 << 0);
	...
	public static int getMaxLength(int typeNumber, int mode, int errorLevel) {
		int t = typeNumber - 1;
		int e = 0;
		int m = 0;
		switch (errorLevel) {
	...
	public static QRPolynomial getErrorPolynomial(int errorCorrectLength) {
		QRPolynomial a = new QRPolynomial(new int[] { 1 });
		for (int i = 0; i < errorCorrectLength; i++) {
			a = a.multiply(new QRPolynomial(new int[] { 1, QRMath.gexp(i) }));
		}
		return a;
	}
	...
	public static int getMode(String s) {
		if (StringUtils.isAlphabetNumeric(s)) {
