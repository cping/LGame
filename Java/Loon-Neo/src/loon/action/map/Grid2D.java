...
public class Grid2D implements LRelease {
	...
	public Grid2D generateMap(boolean allDir) {
		for (; !isGenerated();) {
			...
			if (c1.x >= 0 && c1.x < _mapWidth && c1.y >= 0 && c1.y < _mapHeight) {
				...
				if (cc0.getId() != cc1.getId()) {
					...
					if (door == null) {
						...
						_doors.add(door);
						extendRegion(cc1.getId(), cc0.getId(), c1, allDir);
					}
					door.setOpen(true);
				}
			}
	...
	protected void extendRegion(int fromId, int toId, Vector2f w, boolean allDir) {
		...
		c.setId(toId);
		if (allDir) {
			for (int i = Config.LEFT; i <= Config.TDOWN; i++) {
				Vector2f p = Field2D.getDirection(i);
				extendRegion(fromId, toId, w.add(p), allDir);
			}
		} else {
