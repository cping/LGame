...
public class AmbientCubemap {
	...
	public AmbientCubemap add(final float r, final float g, final float b, final Vector3f direction) {
		return add(r, g, b, direction.x, direction.y, direction.z);
	}

	public AmbientCubemap add(final LColor color, final float x, final float y, final float z) {
		return add(color.r, color.g, color.b, x, y, z);
	}

	public AmbientCubemap add(final LColor color, final Vector3f point, final Vector3f target) {
		return add(color.r, color.g, color.b, target.x - point.x, target.y - point.y, target.z - point.z);
	}

	public AmbientCubemap add(final LColor color, final Vector3f point, final Vector3f target, final float intensity) {
