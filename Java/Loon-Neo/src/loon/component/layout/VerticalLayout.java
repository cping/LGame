...
public class VerticalLayout extends LayoutManager {
	...
	@Override
	public LayoutManager layoutElements(final LayoutPort root, final LayoutPort... children) {
		...
		for (int i = 0; i < children.length; i++) {
			...
			LayoutConstraints currentBoxConstraints = children[i].getBoxConstraints();
			...
			if (hasHeightConstraint(currentBoxConstraints) && currentBoxConstraints.getHeight().hasWidthSuffix()) {
				...
				elementHeight = calcElementHeight(new TArray<LayoutPort>(children), rootBoxHeight,
			...
			currentBox.setX(processHorizontalAlignment(rootBoxX, rootBoxWidth, (int) currentBox.getWidth(),
					currentBoxConstraints));
	...
	private int processWidthConstraints(final int rootBoxWidth, final LayoutConstraints constraints,
	...
	private int processHorizontalAlignment(final int rootBoxX, final int rootBoxWidth, final int currentBoxWidth,
	...
	private int calcElementHeight(final TArray<LayoutPort> children, final int rootBoxHeight,
			final LayoutConstraints boxConstraints, final int boxWidth) {
	...
	private int getMaxNonFixedHeight(final TArray<LayoutPort> elements, final int parentHeight) {
	...
	private boolean isInvalid(final LayoutPort root, final LayoutPort... children) {
