...
public class HorizontalLayout extends LayoutManager {
	...
	@Override
	public final LayoutManager layoutElements(final LayoutPort root, final LayoutPort... children) {
		...
		for (int i = 0; i < children.length; i++) {
			...
			if (boxConstraints.getWidth() != null && boxConstraints.getWidth().hasHeightSuffix()) {
				...
				elementWidth = calcElementWidth(new TArray<LayoutPort>(children), rootBoxWidth, boxConstraints,
						elementHeight);
				if (_allow) {
					box.setWidth(elementWidth);
				}
			} else if (hasHeightConstraint(boxConstraints) && boxConstraints.getHeight().hasWidthSuffix()) {
				elementWidth = calcElementWidth(new TArray<LayoutPort>(children), rootBoxWidth, boxConstraints, 0);
				if (_allow) {
					box.setWidth(elementWidth);
				}
	...
	private int processHeightConstraint(final int rootBoxHeight, final BoxSize box, final LayoutConstraints constraint,
	...
	private int calcElementWidth(final TArray<LayoutPort> children, final int rootBoxWidth,
			final LayoutConstraints boxConstraints, final int elementHeight) {
	...
	private int processVerticalAlignment(final int rootBoxY, final int rootBoxHeight, final BoxSize box,
	...
	private int getMaxNonFixedWidth(final TArray<LayoutPort> elements, final int parentWidth) {
		int maxFixedWidth = 0;
		int fixedCount = 0;
