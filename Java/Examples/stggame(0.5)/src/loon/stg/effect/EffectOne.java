...
public class EffectOne implements LRelease {
	...
	public EffectOne(LTexture[] image, int num) {
		if (sinX == null || cosX == null) {
			...
			cosX = new double[20];
			for (int j = 0; j < 20; j++) {
				sinX[j] = MathUtils.sin((3.1415926535897931f * j) / 10f);
				cosX[j] = MathUtils.cos((3.1415926535897931f * j) / 10f);
			}
		}
		this.time = 0;
		this.drawType = 0;
	...
	protected void drawBreak(GLEx g) {
		...
		arrayX[activeNum] = drawX
				+ MathUtils.abs(MathUtils.random.nextInt() % drawW);
		arrayY[activeNum] = drawY
				+ MathUtils.abs(MathUtils.random.nextInt() % drawH);
