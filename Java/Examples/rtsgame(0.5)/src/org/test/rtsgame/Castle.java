...
public class Castle {
	...
	public Castle(GameContent gameContent, Shape shape, Vector2f tileCenter) {
		...
		int num = 15;
		this.BoundingRectangle = new RectBox((this.position.x) - num, (this.position.y) - num, 2 * num, 2 * num);
		this.AttackRectangle = this.BoundingRectangle;
		this.AttackRectangle.inflate(120, 360);
	}

	public final void Draw(SpriteBatch batch) {
		batch.draw(this.gameContent.castle[this.shape.getValue()],
				(this.position.add(0f, 15f)).sub(this.gameContent.castleOrigin),
				LColorPool.get().getColor(1f, 1f, 1f, 0.8f));
	}
}