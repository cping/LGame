...
import loon.LSystem;
import loon.LTouchCollection;
import loon.LTouchLocation;
import loon.Touch;
...
public class RoleMoveControl implements LRelease {
	...
	public RoleMoveControl(EntityManager screenManager, int levelIndex) {
		...
		this.castles[1] = new Castle(gameContent, Shape.triangle, new Vector2f(
				LSystem.screenRect.width, LSystem.screenRect.height).sub(item));
			...
			for (int j = 0; j < details.ArmyNumber.get(i); j++) {
				...
				this.armies.add(new RoleControl(this.gameContent,
						Shape.triangle, RoleRank.forValue(i), new Vector2f(
								LSystem.screenRect.width, LSystem.screenRect.height).sub(position)));
			}
		}
	...
	public final void HandleInput() {
		...
		Vector2f nextDestPosition = Touch.getLocation().cpy();
		LTouchCollection collection = Touch.getTouchState();
		...
		if (collection.size() > 0) {
			...
			if (Touch.isDown() && (this.mouseOverArmy != null)) {
