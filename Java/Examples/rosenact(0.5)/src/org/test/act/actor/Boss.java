...
public class Boss extends BaseSprite {
	...
	private void cleanOutShot1s() {
		for (BossShot1 shot : this.shot1s) {
			if (shot.visible
					&& (((((shot.Pos.x - shot.Origin.x) + super.getWidth()) < 0f) || ((shot.Pos.x - shot.Origin.x) > LSystem.viewSize.width)) || (((shot.Pos.y - shot.Origin.y) > LSystem.viewSize.height) || (((shot.Pos.y - shot.Origin.y) + super
							.getHeight()) < 0f)))) {
				shot.visible = false;
			}
		}
