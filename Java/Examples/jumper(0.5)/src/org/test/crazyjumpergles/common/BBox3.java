...
public class BBox3 {
	...
	public final boolean contains(BBox3 bb) {
		return (this.is_point_in(bb.vmin) || (this.is_point_in(new Vector3f(bb.vmin.x, bb.vmin.y, bb.vmax.z)) || (this
				.is_point_in(new Vector3f(bb.vmax.x, bb.vmin.y, bb.vmax.z))
				|| (this.is_point_in(new Vector3f(bb.vmax.x, bb.vmin.y, bb.vmin.z)) || (this.is_point_in(bb.vmax)
						|| (this.is_point_in(new Vector3f(bb.vmin.x, bb.vmax.y, bb.vmin.z))
								|| (this.is_point_in(new Vector3f(bb.vmin.x, bb.vmax.y, bb.vmax.z))
										|| this.is_point_in(new Vector3f(bb.vmax.x, bb.vmax.y, bb.vmin.z)))))))));
	}
	...
	public final boolean intersect(Line3 line) {
		...
		if (((((vector.x >= this.vmin.x) && (vector.y >= this.vmin.y))
				&& ((vector.z >= this.vmin.z) && (vector.x <= this.vmax.x)))
				&& ((vector.y <= this.vmax.y) && (vector.z <= this.vmax.z)))
				|| ((((vector2.x >= this.vmin.x) && (vector2.y >= this.vmin.y))
						&& ((vector2.z >= this.vmin.z) && (vector2.x <= this.vmax.x)))
						&& ((vector2.y <= this.vmax.y) && (vector2.z <= this.vmax.z)))) {
			return true;
		}
	...
	public final boolean intersect_closest_point_get(Line3 line, ObjRef<Vector3f> outVec) {
	...
	public final boolean is_point_in(Vector3f p) {
		return ((((p.x >= this.vmin.x) && (p.x <= this.vmax.x)) && ((p.y >= this.vmin.y) && (p.y <= this.vmax.y)))
				&& ((p.z >= this.vmin.z) && (p.z <= this.vmax.z)));
	}

	public final boolean isect_const_x(float x, Line3 l, ObjRef<Vector3f> outVec) {
		if (l.m.x != 0f) {
	...
	public final boolean isect_const_y(float y, Line3 l, ObjRef<Vector3f> outVec) {
		if (l.m.y != 0f) {
	...
	public final boolean isect_const_z(float z, Line3 l, ObjRef<Vector3f> outVec) {
		if (l.m.z != 0f) {
	...
	public enum EClip {
		CLIP_BOTTOM(4), CLIP_FAR(0x20), CLIP_LEFT(1), CLIP_NEAR(0x10), CLIP_RIGHT(2), CLIP_TOP(8);
	...
	public enum EIntersctResult {
		CLIPS(8), CONTAINS(4), ISCONTAINED(2), ISEQUAL(1), OUTSIDE(0);
