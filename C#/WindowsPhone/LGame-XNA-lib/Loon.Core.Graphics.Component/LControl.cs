...
namespace Loon.Core.Graphics.Component {
	...
	public class LControl : LComponent {
		...
		public LControl(int x, int y, string basename, string dot, int bw, int bh,
				int dw, int dh):this(x, y, LTextures.LoadTexture(basename, Loon.Core.Graphics.Opengl.LTexture.Format.LINEAR), LTextures
					.LoadTexture(dot, Loon.Core.Graphics.Opengl.LTexture.Format.LINEAR), bw, bh, dw, dh) {
		...
		private void OnUpdateControlDot(float x, float y) {
			...
			if (this.allowDiagonal) {
				...
				if (LAccelerometer.CheckAngle(0, angle)
						|| LAccelerometer.CheckAngle(360, angle)) {
