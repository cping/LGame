...
namespace Loon.Core.Input
{
    ...
    public static class Key
    {
        ...
        public static bool IsKeyPressed(int key)
        {
            if (key == Key.ANY_KEY)
            {
                return LInputFactory.keys.size > 0 && LInputFactory.m_only_key.IsPressed();
            }
            else
            {
                return LInputFactory.keys.Contains(key) && LInputFactory.m_only_key.IsPressed();
            }
        }
    ...
    public class LInputFactory
    {
        ...
        public static LTouchCollection GetTouchState()
        {
            ...
            touchCollection.SetConnected(useTouchCollection);
        ...
        public bool IsGamePadBackExit
        {
            ...
            set
            {
                LSystem.isBackLocked = value;
            }

        }

        //游戏手柄监听用缓存
        private static LinkedList<XNAGamePadListener> gamePadListeners;

        private static LinkedList<XNAGamePadListener> gamePadListenersToAdd = new LinkedList<XNAGamePadListener>();

        private static LinkedList<XNAGamePadListener> gamePadListenersToRemove = new LinkedList<XNAGamePadListener>();
        ...
        public LInputFactory(LProcess p)
        {
            ...
            gamePadListeners = new LinkedList<XNAGamePadListener>();
        ...
        public static void AddGamePadListener(XNAGamePadListener g)
        {
            gamePadListenersToAdd.AddLast(g);
        }

        public static void RemoveGamePadListener(XNAGamePadListener g)
        {
        ...
        {
            UpdateListeners<XNAGamePadListener>(gamePadListeners, gamePadListenersToAdd, gamePadListenersToRemove);
        }
                ...
                foreach (TouchLocation touch in TouchState)
                {
                    ...
                    switch (touch.State)
                    {
                        case TouchLocationState.Pressed:
                            ...
                            {
                                touchCollection.Add(finalTouch.id, touchX,
                                       touchY);
                            }
                            offsetTouchX = touchX;
                            offsetTouchY = touchY;
                        ...
                        case TouchLocationState.Released:
                            if (useTouchCollection)
                            {
                                touchCollection.Update(finalTouch.id,
                                        LTouchLocationState.Released, touchX,
                                        touchY);
                            }
                            finalTouch.button = Touch.TOUCH_UP;
                            finalTouch.x0 = touchX;
                        ...
                        case TouchLocationState.Moved:
                                    ...
                                    || MathUtils.Abs(offsetTouchY - offsetMoveY) > 5)
                            {
                                ...
                                {
                                    touchCollection.Update(finalTouch.id,
                                            LTouchLocationState.Dragged, touchX,
                                            touchY);
                                }
                                finalTouch.x0 = touchX;
                                finalTouch.y0 = touchY;
                        ...
                        case TouchLocationState.Invalid:
                            ...
                            {
                                touchCollection.Update(finalTouch.id,
                                        LTouchLocationState.Invalid, touchX,
                                       touchY);
                            }
                            finalTouch.button = Touch.TOUCH_UP;
                            break;
        ...
        protected internal readonly static ActionKey m_only_key = new ActionKey(
                ...
                if (!EmptyPad.Equals(button))
                {
                    ...
                    if (!LSystem.isBackLocked)
                    {
                        if (IsPadPressed(Buttons.Back, idx))
                        {
                            ...
                            if (!Guide.IsVisible)
                            {
                                Guide.BeginShowMessageBox("Exit", text, buttons, 0, MessageBoxIcon.Alert, new AsyncCallback(this.BackMessage), null);
                            }
                        }
                    }
                }
        ...
        protected void BackMessage(IAsyncResult r)
        {
            if (Guide.EndShowMessageBox(r) == 0)
            {
                if (LSystem.screenActivity != null)
                {
                    LSystem.screenActivity.Exit();
                }
            }
        }
        ...
        private void RunGamePad(int player)
        {
            ...
            {
                foreach (XNAGamePadListener e in gamePadListeners)
                {
