...
namespace Loon.Action.Map
{
    ...
    public class AStarFinder : Runnable, LRelease
    {
                ...
                bool flag)
        {
            ...
            hashCode = LSystem.Unite(hashCode, heuristic.GetCodeType());
                ...
                bool flag)
        {
            ...
            lock (finderLazy)
            {
                ...
                if (result != null)
                {
                    ...
                    CollectionUtils.AddAll(result, newResult);
                    result = newResult;
                }
                return result;
            }
        ...
        public AStarFinder(AStarFindHeuristic heuristic, bool fly)
        {
            this.flying = fly;
        ...
        private List<Vector2f> Astar(Field2D field, bool flag)
        {
            for (; pathes.Count > 0; )
            {
                ScoredPath spath = (ScoredPath)CollectionUtils.RemoveAt(pathes,0);
                Vector2f current = spath.path[spath.path.Count - 1];
                if (current.Equals(goal))
                {
                    return spath.path;
                }
              
                List<Vector2f> list = field.Neighbors(current, flag);
                int size = list.Count;
                for (int i = 0; i < size; i++)
                {
                    ...
                    List<Vector2f> path = new List<Vector2f>(spath.path);
                    path.Add(next);
        ...
        private void Insert(float score, List<Vector2f> path)
        {
            ...
            for (int i = 0; i < size; i ++)
            {
                ...
                {
                    pathes.Insert(i, new AStarFinder.ScoredPath(score, path));
                    return;
                }
            }
            CollectionUtils.Add(pathes, new AStarFinder.ScoredPath(score, path));
        }
