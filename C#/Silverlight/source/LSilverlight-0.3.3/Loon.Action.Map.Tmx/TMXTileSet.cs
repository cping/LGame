...
namespace Loon.Action.Map.Tmx
{
    public class TMXTileSet : LRelease
    {
        ...
        public TMXTileSet(TMXTiledMap map_0, XMLElement element, bool loadImage)
        {
            this.map = map_0;
            ...
            if (!"".Equals(source))
            {
                try
                {
                    Stream ins0 = Resources.OpenStream(map_0.GetTilesLocation()
                            + "/" + source);
            ...
            if (loadImage)
            {
                string path = map_0.GetTilesLocation() + "/" + fileName;
                LTexture image;
                if (trans != null)
                {
