namespace Loon.Core.Graphics.Device
{
    ...
    public class BitmapDecoder
    {
        ...
        public BitmapDecoder(string file)
            : this(Resources.OpenSource(file), file)
        {
            
        }

        public BitmapDecoder(InputStream ins)
            : this(ins, null)
        {
        }

        public BitmapDecoder(InputStream ins,string file)
        {
            this.width = 0;
            this.height = 0;
            this.format = null;
            ...
            {
                Log.DebugWrite("File not found:" + file);
            }
            catch (IOException)
            {
                Log.DebugWrite("Error on reading:" + file);
            }
            if (this.format == null)
            {
                Log.DebugWrite("Unsuported format !");
            }
        }
