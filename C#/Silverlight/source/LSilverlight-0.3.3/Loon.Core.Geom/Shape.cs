namespace Loon.Core.Geom {
	...
	public abstract class Shape {
		...
		public Shape() {
			pointsDirty = true;
			type = Loon.Core.Geom.ShapeType.DEFAULT_SHAPE;
			scaleX = scaleY = 1f;
		}
		...
		public virtual float Length() {
			return Loon.Utils.MathUtils.Sqrt(x * x + y * y);
		}
		...
		public float[] GetNormal(int index) {
			...
			float len = Loon.Utils.MathUtils.Sqrt((tx * tx) + (ty * ty));
		...
		private float[] GetNormal(float[] start, float[] end) {
			...
			float len = Loon.Utils.MathUtils.Sqrt((dx * dx) + (dy * dy));
			dx /= len;
			dy /= len;
		...
		protected internal virtual void CalculateRadius() {
			...
			boundingCircleRadius = Loon.Utils.MathUtils.Sqrt(boundingCircleRadius);
		}
		...
		public void SetRotation(float r) {
			if (rotation != r) {
				this.CallTransform(Loon.Core.Geom.Matrix.CreateRotateTransform(
						rotation = (r / 180f * Loon.Utils.MathUtils.PI), this.center[0],
						this.center[1]));
			}
		}
		...
		public void SetRotation(float r, float x_0, float y_1) {
			if (rotation != r) {
				this.CallTransform(Loon.Core.Geom.Matrix.CreateRotateTransform(
						rotation = (r / 180f * Loon.Utils.MathUtils.PI), x_0, y_1));
			}
		}
	
		public float GetRotation() {
			return (rotation * 180f / Loon.Utils.MathUtils.PI);
		}
				 ...
				 lock (points) {
								...
								if (size > 0) {
									...
									for (int i = 0; i < size / 2; i++) {
										maxX = Loon.Utils.MathUtils.Max(points[i * 2], maxX);
										maxY = Loon.Utils.MathUtils.Max(points[(i * 2) + 1], maxY);
										minX = Loon.Utils.MathUtils.Min(points[i * 2], minX);
										minY = Loon.Utils.MathUtils.Min(points[(i * 2) + 1], minY);
									}
								}
								pointsDirty = false;
								trianglesDirty = true;
