namespace Loon.Core.Graphics.OpenGL
{
    ...
    public class LTexture : LRelease
    {
        ...
        internal static Texture2D LoadFromResource(string name)
        {
            ...
            Texture2D tex = Texture2D.FromStream(GLEx.Device, asm.GetManifestResourceStream(String.Format("Loon.{0}", name)));
        ...
        public LTexture(string @resName)
        {
            ...
            else
            {
                if (LSystem.screenProcess != null)
                {
                    ...
                    {
                        tex2d = LSilverlightPlus.Get.Load<Texture2D>(StringUtils.ReplaceIgnoreCase(resName, ".xnb", ""));
                        width = tex2d.Width;
                        height = tex2d.Height;
                    }
                }
                ...
                if (!isLoaded)
                {
                    ...
                    else
                    {
                        ...
                        if (!isExt)
                        {
                            if (tex2d == null)
                            {
                                this.tex2d = LSilverlightPlus.Get.Load<Texture2D>(fileName);
                            }
                        }
                        else
                        {
