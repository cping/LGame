...
namespace Loon.Utils
{
    public sealed class FileUtils
    {
        ...
        /// </summary>
        ///
        /// <param name="filePath">文件的路径，可以是相对路径也可以是绝对路径</param>
        /// <returns>对应的文件名</returns>
        ...
        /// <param name="fileName"></param>
        /// <returns></returns>
        static public string ToformatPath(string fileName)
        {
            return new FileInfo(fileName).FullName;
        }
        ...
        /// @function 得到文件的类型。
        /// <param name="fileName">文件名</param>
        /// <returns>文件名中的类型部分</returns>
        public static string ToTypePart(string fileName)
        {
            int point = fileName.LastIndexOf('.');
            int length = fileName.Length;
            return (point == -1 || point == length - 1) ? "" : fileName.Substring(point + 1, (length) - (point + 1));
        }
        ...
        /// <returns>文件名中的类型部分</returns>
        public static string ToFileType(FileInfo file)
        {
            return ToTypePart(file.Name);
        }
        ...
        public static string ToNamePart(string fileName)
        {
            ...
            else if (point == length - 1)
            {
                ...
                if (secondPoint == -1)
                {
                    ...
                    {
                        return fileName.Substring(0, (point) - (0));
                    }
                }
                else
                {
                    return fileName.Substring(secondPoint + 1, (point) - (secondPoint + 1));
                }
            }
            else
            {
        ...
        ///
        /// <param name="fileName">文件名</param>
        /// <returns>父路径，不存在或者已经是父目录时返回""</returns>
        public static string ToPathPart(string fileName)
        {
            ...
            else if (point == length - 1)
            {
                ...
                {
                    return fileName.Substring(0, (secondPoint) - (0));
                }
            }
            else
            {
        ...
        /// <returns>路径分隔符在路径中首次出现的位置，没有出现时返回-1。</returns>
        ...
        /// <returns>路径分隔符在路径中指定位置后首次出现的位置，没有出现时返回-1。</returns>
        ...
        ///
        /// <param name="fileName">文件路径</param>
        /// <returns>路径分隔符在路径中最后出现的位置，没有出现时返回-1。</returns>
        ...
        /// <param name="fileName">文件路径</param>
        /// <param name="fromIndex">开始查找的位置</param>
        /// <returns>路径分隔符在路径中指定位置前最后出现的位置，没有出现时返回-1。</returns>
        ...
        /// </summary>
        ///
        /// <param name="pathName">目录名</param>
        /// <param name="fileName">文件名</param>
        /// <returns>得到文件名相对于目录名的相对路径，目录下不存在该文件时返回文件名</returns>
        public static string ToSubpath(string pathName, string fileName)
        {
            return (fileName.IndexOf(pathName) != -1) ? fileName.Substring(fileName
                            .IndexOf(pathName) + pathName.Length + 1) : fileName;
        }
        ...
        ///
        /// <returns></returns>
        public static string ToFileRoot()
        { // 返回所有系统目录
            FileInfo[] dvs = new DirectoryInfo(".").GetFiles();
            string[] rootname = new string[dvs.Length];
            StringBuilder path = new StringBuilder();
            for (int i = 0; i < rootname.Length; i++)
            {
                rootname[i] = System.IO.Path.GetFullPath(dvs[i].Name);
        ...
        /// <param name="file"></param>
        /// <returns></returns>
        public static string[] ToList(DirectoryInfo file)
        {
