...
namespace loon.opengl
{
    public class GL20 : GL
    {
        ...
        public GL20(GraphicsDevice d, int w, int h)
        {
        ...
        public virtual Texture2D SetTextureData(int texid, uint[] data)
        {
            Texture2D texture = null;
            if (_textures.ContainsKey(texid))
            {
                texture = _textures.Get(texid);
                if (texture != null)
                {
        ...
        public virtual uint[] GetTextureData(int texid, uint[] rawTexture)
        {
            if (_textures.ContainsKey(texid))
            {
                Texture2D texture = _textures.Get(texid);
                if (texture != null)
                {
        ...
        public virtual uint[] GetTextureData(int texid)
        {
            if (_textures.ContainsKey(texid))
            {
                Texture2D texture = _textures.Get(texid);
                if (texture != null)
                {
                    uint[] rawTexture = new uint[texture.Width * texture.Height];
                    texture.GetData(rawTexture);
                    return rawTexture;
                }
            }
        ...
        public int CreateTexture(float w, float h, bool mipmap)
        {
            Texture2D texture = new Texture2D(_graphicsDevice, (int)w, (int)h, mipmap, SurfaceFormat.Color);
            return CreateTexture(texture);
        }

        public int CreateTexture(float w, float h, bool mipmap, TextureType textype)
        {
            Texture2D texture = new Texture2D(_graphicsDevice, (int)w, (int)h, mipmap, SurfaceFormat.Color);
            return CreateTexture(texture, textype);
        }
