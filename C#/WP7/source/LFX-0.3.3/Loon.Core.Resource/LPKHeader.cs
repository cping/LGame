...
namespace Loon.Core.Resource
{
    public class LPKHeader
    {
        ...
        public LPKHeader(byte[] p, float v1, long t1)
        {
            this.version = 1.0F;
            this.tables = 0;
            this.password = new byte[LPKHeader.LF_PASSWORD_LENGTH];
            for (int i = 0; i < LPKHeader.LF_PASSWORD_LENGTH; this.password[i] = p[i], i++)
                ;
            this.version = v1;
            this.tables = t1;
        }
        ...
        {
            this.password = StringUtils.GetBytes(NumberUtils
                            .AddZeros(pass, LPKHeader.LF_PASSWORD_LENGTH));
        }

        public void SetPassword(byte[] p)
        {
            for (int i = 0; i < LPKHeader.LF_PASSWORD_LENGTH; p[i] = p[i], i++)
                ;
        }
