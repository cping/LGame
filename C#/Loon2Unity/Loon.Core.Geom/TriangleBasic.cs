...
namespace Loon.Core.Geom {
	...
	public class TriangleBasic : Triangle {
		...
		public float[] GetPolyPoint(int index) {
			return new float[] { poly.Get(index).GetX(), poly.Get(index).GetY() };
		}
		...
		public virtual int GetTriangleCount() {
			if (!tried) {
				throw new Exception("this not Triangle !");
			}
			return tris.Size() / 3;
		}
		...
		private class PointList {
			...
			public void Remove(Point point) {
                CollectionUtils.Remove(points, point);
			}
