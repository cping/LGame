...
public class WWMath {
	...
	public static final LatLon LONGITUDE_OFFSET_180 = LatLon
			.fromDegrees(0, 180);
	 ...
	 * 
	 * @param coords
	 *            the coordinates. This method returns null if this argument is
	 *            null.
	 * @param stride
	 *            the number of floats between successive points. 0 indicates
	...
	public static Vec4 computeBufferNormal(FloatBuffer coords, int stride) {
		...
		return verts != null ? WWMath.computeTriangleNormal(verts[0], verts[1],
				verts[2]) : null;
	}
	 ...
	 *         could not be found.
	 */
	public static Vec4[] findThreeIndependentVertices(FloatBuffer coords,
			int stride) {
		...
		for (k += xstride; k < coords.limit(); k += xstride) {
			...
			// if c is not coincident with a or b, and the vectors ab and bc are
			// not colinear, break and return a, b, c
			if (!((c.x == a.x && c.y == a.y && c.z == a.z) || (c.x == b.x
					&& c.y == b.y && c.z == b.z))) {
	...
	public static String computeWindingOrderOfLocations(
		...
		if (LatLon.locationsCrossDateLine(locations))
			iter = LatLon.makeDatelineCrossingLocationsPositive(locations)
					.iterator();
		...
		while (iter.hasNext()) {
			...
			area += location.getLongitude().degrees
					* nextLocation.getLatitude().degrees;
			area -= nextLocation.getLongitude().degrees
		...
		if (!location.equals(firstLocation)) {
			area += location.getLongitude().degrees
					* firstLocation.getLatitude().degrees;
			area -= firstLocation.getLongitude().degrees
					* location.getLatitude().degrees;
		}
	 ...
	 */
	public static String computeWindingOrderOfVertices(
			Iterable<? extends Vec4> points) {
		if (points == null) {
			String message = Logging.getMessage("nullValue.IterableIsNull");
			Logging.logger().severe(message);
	 ...
	 * 
	 * @param points
	 *            the Iterable of points for which to compute the principal
	 *            axes.
	 * 
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if the points Iterable is null.
	 */
	public static Vec4[] computePrincipalAxes(Iterable<? extends Vec4> points) {
		...
		// Compute the covariance matrix of the specified points Iterable. Note
		// that Matrix.fromCovarianceOfVertices
		// returns null if the points Iterable is empty, or if all of the points
		// are null.
		...
		// by definition, we can safely use the method
		// Matrix.computeEigensystemFromSymmetricMatrix3().
		final double[] eigenValues = new double[3];
		final Vec4[] eigenVectors = new Vec4[3];
		Matrix.computeEigensystemFromSymmetricMatrix3(covariance, eigenValues,
				eigenVectors);

		// Compute an index array who's entries define the order in which the
		// eigenValues array can be sorted in
		// ascending order.
		...
		// Return the normalized eigenvectors in order of decreasing eigenvalue.
		// This has the effect of returning three
		// normalized orthognal vectors defining a coordinate system, which are
		// sorted from the most prominent axis to
		// the least prominent.
		return new Vec4[] { eigenVectors[indexArray[2]].normalize3(),
	 ...
	 * 
	 * @param coordinates
	 *            the buffer containing the point coordinates for which to
	 *            compute the principal axes.
	 * @param stride
	 *            the number of elements between the first coordinate of
	 *            consecutive points. If stride is 3, this interprets the buffer
	 *            has having tightly packed XYZ coordinate tuples.
	 * 
	 * @return the normalized principal axes of the points, sorted from the most
	 *         prominent axis to the least prominent.
	 * 
	 * @throws IllegalArgumentException
	 *             if the buffer is null, or if the stride is less than three.
	 */
	public static Vec4[] computePrincipalAxes(BufferWrapper coordinates,
		...
		Matrix covariance = Matrix
				.fromCovarianceOfVertices(coordinates, stride);
		if (covariance == null)
			return null;
	 ...
	 * 
	 * @param locations
	 *            the locations which define the geographic polygon.
	 * 
	 * @return true if the polygon defines a closed loop, and false otherwise.
	 * 
	 * @throws IllegalArgumentException
	 *             if the locations are null.
	 */
	...
	public static boolean isPolygonClosed2(Iterable<? extends Vec4> points) {
		...
		return (lastPoint != null) && (lastPoint.x == firstPoint.x)
	 ...
	 *            the location
	 * @param locations
	 *            the list of positions describing the polygon. Last one should
	...
	public static boolean isLocationInside(LatLon location,
		...
		// Test for even/odd number of intersections with a constant latitude
		// line going through the given location.
		...
		while (iter.hasNext()) {
			...
			if (((p2.getLatitude().degrees <= location.getLatitude().degrees && location
					.getLatitude().degrees < p1.getLatitude().degrees) || (p1
					.getLatitude().degrees <= location.getLatitude().degrees && location
					.getLatitude().degrees < p2.getLatitude().degrees))
					&& (location.getLongitude().degrees < (p1.getLongitude().degrees - p2
							.getLongitude().degrees)
							* (location.getLatitude().degrees - p2
									.getLatitude().degrees)
	 ...
	 *            the third point.
	 * @param centerOut
	 *            preallocated array to hold the circle's center.
	 * @param axisOut
	 *            preallocated array to hold the circle's axis.
	 * @param radiusOut
	 *            preallocated array to hold the circle's radius.
	 * 
	 * @return true if the computation was successful; false otherwise.
	 * 
	 * @throws IllegalArgumentException
	 *             if <code>p0</code>, <code>p1</code>, or <code>p2</code> is
	 *             null
	 */
	public static boolean computeCircleThroughPoints(Vec4 p0, Vec4 p1, Vec4 p2,
			Vec4[] centerOut, Vec4[] axisOut, double[] radiusOut) {
		...
		// the three points are collinear -- no circle with finite radius is
		// possible
		if (radiusSquared < 0d)
			return false;
	 ...
	 * 
	 * @param line
	 *            the line to intersect with the polytope.
	 * @param planes
	 *            the planes defining the polytope. Each plane's normal must
	 *            point away from the the polytope, i.e. each plane's positive
	 *            halfspace is outside the polytope. (Note: This is the opposite
	 *            convention from that of a view frustum.)
	 * 
	 * @return the points of intersection, or null if the line does not
	 *         intersect the polytope. Two points are returned if the line both
	 *         enters and exits the polytope. One point is retured if the line
	 *         origin is within the polytope.
	 * 
	 * @throws IllegalArgumentException
	 *             if the line is null or ill-formed, the planes array is null
	 *             or there are fewer than three planes.
	 */
	public static Intersection[] polytopeIntersect(Line line, Plane[] planes) {
		...
		// Algorithm from "3-D Computer Graphics" by Samuel R. Buss, 2005,
		// Section X.1.4.

		// Determine intersection with each plane and categorize the
		// intersections as "front" if the line intersects
		// the front side of the plane (dot product of line direction with plane
		// normal is negative) and "back" if the
		// line intersects the back side of the plane (dot product of line
		// direction with plane normal is positive).
		...
		for (Plane plane : planes) {
			...
			{
				double pdn = p.dot3(n);
				if (pdn > d) // is line in positive halfspace (in front of) of
								// the plane?
					return null; // no intersection
				else {
					if (pdn == d)
						isTangent = true; // line coincident with plane
					continue; // line is in negative halfspace; possible
								// intersection; check other planes
				}
			}

			// Determine whether front or back intersection.
			double a = (d - p.dot3(n)) / s;
			if (u.dot3(n) < 0) // line intersects front face and therefore
								// entering polytope
			{
				if (a > fMax) {
					if (a > bMin)
		...
		// Compute the Cartesian intersection points. There will be no more than
		// two.
		if (fMax >= 0) // intersects frontface and backface; point origin is
			...
			// intersects backface only; point origin is within the polytope
			return new Intersection[] { new Intersection(p.add3(u
					.multiply3(bMin)), isTangent) };
	}
	 ...
	 * may be used as the source <code>buffer</code> in a call to
	 * {@link javax.media.opengl.GL#glDrawElements(int, int, int, java.nio.Buffer)}
	 * , where <code>mode</code> is
	 * {@link javax.media.opengl.GL#GL_TRIANGLE_STRIP}, <code>count</code> is
	 * the number of elements remaining in the buffer, and <code>type</code> is
	 * {@link javax.media.opengl.GL#GL_UNSIGNED_INT}.
	 * <p/>
	 * For details the drawing OpenGL primitives, see <a
	 * href="http://www.glprogramming.com/red/chapter02.html#name14"
	 * >http://www.glprogramming.com/red/chapter02.html#name14</a>.
	 * 
	 * @param width
	 *            the patch width, in vertices.
	 * @param height
	 *            the patch height, in vertices.
	 * 
	...
	public static IntBuffer computeIndicesForGridInterior(int width, int height) {
		...
		IntBuffer buffer = NativeSupport.newIntBuffer(numIndices);
	 ...
	 * used as the source <code>buffer</code> in a call to
	 * {@link javax.media.opengl.GL#glDrawElements(int, int, int, java.nio.Buffer)}
	 * , where <code>mode</code> is {@link javax.media.opengl.GL#GL_LINE_STRIP},
	 * <code>count</code> is the number of elements remaining in the buffer, and
	 ...
	 * 
	 * @param indices
	 *            indices into the vertex buffer defining a triangle strip.
	 * @param vertices
	 *            buffer of vertex coordinate tuples used to compute the normal
	 *            coordinates.
	 * @param normals
	 *            buffer of normal coordinate tuples that receives the normal
	 *            coordinates, or null to create a new buffer to hold the normal
	 *            coordinates.
	 * 
	 * @return buffer of normal coordinate tuples.
	 * 
	 * @throws IllegalArgumentException
	 *             if either the index buffer or the vertex buffer is null.
	 */
	public static FloatBuffer computeNormalsForIndexedTriangleStrip(
			IntBuffer indices, FloatBuffer vertices, FloatBuffer normals) {
		if (indices == null) {
		...
		// If the normal buffer is null, create a new one with the capacity to
		// store the same number of vertices as
		// the vertex buffer. Otherwise, initialize the normal buffer by setting
		// all normal coordinate to zero.
		if (normals == null)
			normals = NativeSupport.newFloatBuffer(3 * numVertices);
		else {
		...
		// Compute the normal for each face, then add that normal to each
		// individual vertex of the face. After this step
		// each normal contains the accumulated normal values from each face it
		// contributes to.
		...
		for (int i = 2; i < numIndices; i++) {
			...
			// Triangle strips alternate between counter-clockwise and clockwise
			// winding order each triangle. Triangles
			// starting with an even index are clockwise, while those starting
			// with an odd index are counter-clockwise.
			// Reverse the face's vertex order for even triangles to ensure that
			// all faces have counter-clockwise
			// ordering.
			if ((i % 2) != 0) {
				int tmp = triangle[0];
			...
			addTriangleNormal(3 * triangle[0], 3 * triangle[1],
					3 * triangle[2], vertices, normals);
		}
		...
		// Normalize each tuple. Each normal contains the accumulated normal
		// values from each face it contributes to
		// Normalizing the tuple averages the accumulated normals from each
		// face, and ensures that the normal has unit
		// length.
		for (int i = 0; i < numVertices; i++) {
			// Normalizes the tuple and the buffer's position then advances to
			// the next tuple.
			normalize3(normals);
		}
	 ...
	 * 
	 * @param a
	 *            the first tuple's starting position.
	 * @param b
	 *            the second tuple's starting position.
	 * @param c
	 *            the third tuple's starting position.
	 * @param vertices
	 *            buffer of vertex coordinate tuples used to compute the normal
	 *            coordinates.
	 * @param normals
	 ...
	 *            coordinates.
	 */
	protected static void addTriangleNormal(int a, int b, int c,
	 ...
	 * @param y0
	 *            the y coordinate of the first point, relative to an upper-left
	 *            origin.
	 * @param x1
	 *            the x coordinate of the second point.
	 * @param y1
	 *            the y coordinate of the second point, relative to an
	 *            upper-left origin.
	 * 
